...
Aziel Runtime is a node-meshed orchestration suite of MCP-connected software designed to coordinate specialized tools through a shared, security-gated runtime while preserving provenance, chain-of-custody, temporal integrity, and auditable execution. It is designed so MCP Softwares run through the FragGate door, mint receipts, and coordinate on the mesh as separate products with designed compartmentalization. Checkable surfaces: FragGate list, describe, and call; hashed catalog `GET /v1/software`; SHA-256 `engine_digest` and session receipts; typed refuse codes `FG-STUB` / `FG-HALLUC-TOOL`. Source maps: [`docs/2.0/INSPECT.md`](2.0/INSPECT.md).
...
FAQ brief (`What Aziel Eliab does`) is machine-only: Softwares sentence + why (`Understand the work…`) + Softwares list (name + one_line + Worker URL, including The ARK local deniable vault at https://ark-download-tracker.vibelock.workers.dev/) + research AZDOC addendum + public-engineering hardware addendum. Cite azielcorpuslibrary.net records. Do not invent DOIs. Library live ~326 records. Person `@id` remains data.
