...
| Y6 Live Nodes rollup | **CLOSED (1.9.1 split)** — then `live_nodes` was Softwares `{slug}-worker` only (`mesh_*` = `ephemeral_nodes`). **OPERATOR LAW (later):** public `live_nodes` is living downloaded Softwares instances; `{slug}-worker` fan-out is `software_nodes`. Downloads are not live. |
