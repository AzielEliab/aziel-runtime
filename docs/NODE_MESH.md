...
- [BAN-SURVIVAL-1.0](designs/BAN-SURVIVAL-1.0.md) — mutual backup: live multi-front ↔ cold shelves; one banned door is not last tip gone; one dead shelf is not last door gone; live-node API SLOT (no open proxy)
...
- **Public Live Nodes (`live_nodes` / `rollup.mesh`):** how **big the node mesh is**. A counted node is any join/heartbeat roster entry that is **active** (presence `live`) **or inactive** (presence `locked`), and **not isolated**. Isolated nodes stay out of Live Nodes and remain on `isolated_nodes`. Short UI label stays **Live Nodes**. This is real mesh presence — **not** catalog size and **not** cumulative downloads. Zero is honest when the roster is empty.
- **`software_nodes` / `rollup.software`:** while suite-presence is LIVE, this Worker fans out `join` / `heartbeat` for every live Softwares product Worker (`node_id` `{slug}-worker`, no `|`) on cron (`*/2 * * * *`) or request-path. Presence TTL is **5 minutes**. GET still never enables extra radios. That roster is **software_nodes** (plus `software_live_nodes` / `software_locked_nodes` / `software_isolated_nodes`). It must **not** be used **alone** as the public Live Nodes number. Non-isolated workers are real mesh presence, so they add to mesh size; instance / named / `mesh_*` joins add too.
- **`mesh_join` contract:** `product` is required (catalog slug). Optional `node_id` must be exactly **8–80** chars matching **`[a-z0-9._-]+`** (full string; no uppercase). `presence` must be **`live`** (default), **`locked`**, or **`isolated`**. Join creates **additive** presence bound to a **strict 5-minute TTL**. `mesh_heartbeat` (or suite fan-out refresh) resets that window. Miss it and the node is **automatically dropped** from the live roster. When transmission radios are powered down or suite radios are not enabled, join/heartbeat/broadcast refuse **`MESH-OFF`** (same code on HTTP and MCP). Read paths stay honest. Public `mesh_disable` still cannot turn suite-presence off.
...
- **BAN-SURVIVAL-1.0:** one banned public door (`workers.dev`, `/mcp`, a FragGate path) is a surface death, not last tip gone. Live multi-front and cold shelves back each other up. Clients try the next LIVE custom-domain hub `/runtime`, then remaining LIVE paths, then shelf tip-hash if every live front is gone. Vice versa: a dead shelf does not kill the named fronts. Live-node API is SLOT (QNM roster is presence, not exec). Never invent a live door. `GET /survival`. Paper: [BAN-SURVIVAL-1.0](designs/BAN-SURVIVAL-1.0.md).
...
| --- | --- | --- | --- |
| GET | `/v1/mesh` | — | `enabled`, `bearers` (Worker rollup: `suite-presence`), `channel_plane` / `channels` (`wifi` / `bluetooth` / `rf` / `photon` ON as cites), `vpn: true` / `public_vpn: true` / `default_vpn_backend: "azvpn"` (cite-only; never opens), `live_nodes` / `rollup.mesh` = mesh size (active + inactive, exclude isolated), `active_nodes` / `inactive_nodes`, `isolated_nodes`, `software_nodes` / `rollup.software` = `{slug}-worker` roster, `live_nodes_note`, `mesh_default: "on"`. **Never enables extra radios.** Downloads are not live. Channel plane ≠ kernel VPN. |
...
3. On the human UI, show a small **QNM rollup** strip (counts only):
   ...
   - Show **Live Nodes · N** from `live_nodes` / `rollup.mesh` (mesh size = active + inactive, not isolated). Tooltip / note: use `live_nodes_note` from the same JSON — do not fork copy. Do **not** display `software_nodes` alone as Live Nodes.
   - `software_nodes` / `rollup.software` is the `{slug}-worker` suite roster. Hubs may cite it separately; it must not be used alone as the Live Nodes pill.
   - Do not treat the poll as enable. Do not show suite “mesh off” / “Default OFF” copy.
   - A **running downloaded Softwares instance** should `POST /v1/mesh/join` with `{ "product": "<slug>", "node_id": "<instance-id>", "presence": "live" }` then heartbeat about once a minute. Instance `node_id` must **not** be `{slug}-worker` (that id is the suite product Worker). Avoid `|` in `node_id`. Isolated presence does not count as Live Nodes.
   - Product Workers may still heartbeat `{ "product": "<slug>", "node_id": "<slug>-worker" }` — that is `software_nodes`. Non-isolated workers also add to mesh-size Live Nodes; do not treat `software_nodes` alone as the pill.
...
- Read-only suite-presence is **ON by default**. Public Live Nodes do not need a manual enable. They stay **0** until a real join/heartbeat roster exists (never invent; downloads are not live).
