...
- **Ban survival** — \`GET ${host}/survival\` cites BAN-SURVIVAL-1.0. If \`/mcp\` or workers.dev is blocked: try named hub \`/runtime\` (same FragGate door), then read surfaces, then cold tip-hash. Never invent a live door.
...
${compatibleClientsHtmlItems()}
  ...
  <ul>
    ...
    <li><strong>Ban survival</strong> — <code>GET ${host}/survival</code> cites BAN-SURVIVAL-1.0. If <code>/mcp</code> or workers.dev is blocked: try named hub <code>/runtime</code>, then read surfaces, then cold tip-hash. Never invent a live door.</li>
  </ul>`;
}
