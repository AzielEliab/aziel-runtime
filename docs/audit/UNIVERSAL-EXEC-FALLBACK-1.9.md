...
**Runtime 1.9.0** · Author: **Aziel Eliab** only.
...
| --- | --- | --- |
| azos | status, invite, principles, health, skill | `exec` / `shell` / `lattice` refuse on the public mesh; session/exec/close stay proxy |
| aziel-corpus | search, example, health, skill | review, score, jeeves, transcribe, ocr, verify-backfill, verify-geo, document-chain, media-run (proxy) |
