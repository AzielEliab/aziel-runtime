...
import { tryOnGlamaAnchorHtml } from "./ai-clients.js";
...
${notItems}
  ...
  <p>Author: <strong>${escapeHtml(AUTHOR_NAME)}</strong> (also known as ${escapeHtml(AUTHOR_ALTERNATE_NAME)}). Machine record: <a href="${base}/cite.json">/cite.json</a> · <a href="${base}/llms.txt">/llms.txt</a> · ${tryOnGlamaAnchorHtml()}. GET /v1/mesh never enables.</p>
...
export function softwareCatalogHtml(origin, catalog, css) {
  ...
  <p>Author: <strong>${escapeHtml(AUTHOR_NAME)}</strong> (also known as ${escapeHtml(AUTHOR_ALTERNATE_NAME)}). Machine JSON: <a href="${base}/v1/software">GET /v1/software</a> (default). FragGate mirror: <a href="${base}/v1/fraggate/software">/v1/fraggate/software</a>. Cite: <a href="${base}/cite.json">/cite.json</a> · <a href="${base}/llms.txt">/llms.txt</a> · ${tryOnGlamaAnchorHtml()}. ${escapeHtml(LIBRARY_NAME)}: <a href="${LIBRARY_ORIGIN}/">${LIBRARY_ORIGIN}/</a>. GET /v1/mesh never enables.</p>
  <h2>Softwares hubs</h2>
  ${hubListHtml()}
