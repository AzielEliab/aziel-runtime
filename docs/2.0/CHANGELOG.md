...
- Additive COLD-MULTI-SHELF-1.0 cite (`GET /shelves`) matches live corpus#96 `/shelves` honesty: Plane A 5 published surfaces / 2 family radii / 1 independent live; Plane B Codeberg PASS still SLOT; Zenodo refused; `doi` null; Plane C USB SLOT. Runtime is the same Plane A tunnel, not a sixth surface. No new MCP tool. Remain-OFF untouched.
