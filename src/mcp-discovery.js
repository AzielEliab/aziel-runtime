...
export function mcpServerCard(origin, env = {}) {
  ...
  return {
    ...
    tools: {
      ...
      summary:
        "36 live MCP tools on POST /mcp tools/list. First call: the Softwares tool. The door runs first. ChainLock stamps when the call needs a ledger. Diagnostics: fraggate_list → fraggate_describe → fraggate_call. ChainLock and memory are append-only.",
