...
assert.equal(CAP7_AZNET.hosted_endpoints.hosted_land, "live");
assert.equal(CAP7_AZNET.hosted_endpoints.attested, true);
assert.equal(CAP7_AZNET.hosted_endpoints.resolves_to_hub, false);
assert.equal(CAP7_AZNET.hosted_endpoints.hosted_mcp, "live");
assert.equal(CAP7_AZNET.hosted_endpoints.status, "live");
assert.equal(CAP7_AZNET.radio_phy, false);
assert.equal(CAP7_AZNET.resolves_to_hub, false);
assert.equal(CAP7_AZNET.standard_internet_reaches_cap7, false);
...
assert.equal(CAP7_AZNET.shuffle.public_worker_shuffle, "live");
assert.equal(CAP7_AZNET.shuffle.hosted_update, "live");
...
assert.equal(cap7BrowserSubset().length, 0);
assert.deepEqual(cap7RealDuplications(), ["azgrid", "azcloak", "azvault", "azshift"]);
assert.deepEqual(cap7FalseSites(), ["azbooth", "azflag", "azstandby"]);
assert.equal(cap7AznetRemainder().length, 3);
assert.equal(siteForMirageNode(1).id, "azgrid");
assert.equal(siteForMirageNode(1).mesh_name, "azgrid.az");
assert.equal(siteForMirageNode(1).mesh_name_icann, "live");
assert.equal(siteForMirageNode(1).hosted_status, "live");
assert.equal(siteForMirageNode(1).false_site, false);
assert.equal(siteForMirageNode(1).internet_reachable, false);
...
assert.ok(CAP7_SITES.every((s) => s.resolves_to_hub === false && s.name_may_change === true && s.is_live_door === false && s.hosted_status === "live" && s.design_of === "hub_designs" && s.public_icann === false && s.internet_reachable === false));
assert.equal(CAP7_SITES.filter((s) => s.false_site).length, 3);
...
assert.equal(judgeFakeCap7Host({ cap7_hosted_endpoint_live: true }).accept, true);
assert.equal(judgeFakeCap7Host({ fake_icann_az: true }).reason, REFUSE.NO_FAKE_CAP7_HOST);
assert.equal(judgeFakeCap7Host({ standard_internet_reaches_cap7: true }).reason, REFUSE.NO_FAKE_CAP7_HOST);
assert.equal(judgeFakeCap7Host({ hosted_mcp: "slot" }).reason, REFUSE.NO_FAKE_CAP7_HOST);
assert.equal(judgeFakeCap7Host({ hosted_land: "slot" }).reason, REFUSE.NO_FAKE_CAP7_HOST);
assert.equal(judgeFakeCap7Host({ resolves_to_hub: true }).reason, REFUSE.NO_FAKE_CAP7_HOST);
...
assert.equal(applyBanSurvival({ cap7_is_mcp: true }).ok, true);
...
assert.equal(applyBanSurvival({ public_shuffle_live: true }).ok, true);
assert.equal(applyBanSurvival({ public_worker_shuffle: "slot" }).ok, false);
assert.equal(applyBanSurvival({ standard_internet_reaches_cap7: true }).ok, false);
...
assert.equal(cite.cap7_aznet.hosted_endpoints.status, "live");
...
assert.equal(cite.cap7_aznet.shuffle.public_worker_shuffle, "live");
assert.equal(cite.cap7_aznet.az_domains.internet_reachable, true);
assert.equal(cite.cap7_aznet.az_domains.resolves_to_hub, true);
assert.equal(cite.cap7_aznet.standard_internet_reaches_cap7, false);
...
assert.equal(cite.cap7_aznet.hosted_endpoints.hosted_mcp, "live");
...
assert.equal(doc.cap7_aznet.hosted_endpoints.status, "live");
...
assert.doesNotMatch(llms, /Never invent a live door/);
assert.doesNotMatch(llms, /Never claim a banned host is LIVE/);
...
assert.match(BAN_SURVIVAL_RULE, /NO-LIE-NO-REWRITE-1\.0/);
assert.match(BAN_SURVIVAL_RULE, /Three layers/);
assert.match(BAN_SURVIVAL_RULE, /docs\/designs\/NO-LIE-NO-REWRITE-1\.0\.md/);
assert.doesNotMatch(BAN_SURVIVAL_RULE, /Never lie to survive|Never invent a live door|Never fake a Cap-7 hosted endpoint/);
...
assert.equal(body.cap7_aznet.hosted_endpoints.status, "live");
...
assert.equal(landed.public_worker_shuffle, "live");
assert.equal(landed.update.status, "live");
assert.equal(landed.standard_internet_reaches_cap7, false);
assert.equal(landed.az_domains.show, 1);
assert.equal(landed.update.hosted_url, null);
assert.equal(landed.update.is_live_door, false);
assert.equal(landed.hosted_mcp, "live");
assert.equal(landed.hosted_land, "live");
assert.equal(landed.attested, true);
assert.equal(landed.resolves_to_hub, false);
assert.equal(landed.name_set_sot, "miragegrid");
assert.equal(landed.land.hosted_mcp, "live");
assert.ok(cap7SiteNames().includes(landed.land.id));
assert.ok(cap7FactoryMeshNames().includes(landed.land.mesh_name));
assert.equal(landed.land.mesh_name_icann, "live");
assert.equal(landed.land.hosted_status, "live");
assert.equal(landed.land.public_icann, false);
assert.equal(landed.land.internet_reachable, false);
...
const fakeLive = await landCap7Shuffle({ standard_internet_reaches_cap7: true });
assert.equal(fakeLive.ok, false);
assert.equal(fakeLive.code, REFUSE.NO_FAKE_SHUFFLE_LIVE);
const fakeMcp = await landCap7Shuffle({ hosted_mcp: "slot" });
assert.equal(fakeMcp.ok, false);
assert.equal(fakeMcp.hosted_mcp, "live");
assert.equal(fakeMcp.resolves_to_hub, false);
assert.equal(fakeMcp.standard_internet_reaches_cap7, false);
...
assert.equal(shuffleBody.result.update.status, "live");
assert.equal(shuffleBody.result.standard_internet_reaches_cap7, false);
...
assert.equal(shuffleBody.result.land.mesh_name_icann, "live");
assert.equal(shuffleBody.result.land.hosted_status, "live");
assert.equal(shuffleBody.result.false_site_count, 3);
