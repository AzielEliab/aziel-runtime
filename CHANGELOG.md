...
- Additive INGEST-RECEIPT-1.0 fabric under **CROSS-NETWORK-SURVIVAL**: crawlers/indexes are shelves so tips survive network death. Survival is bytes↔hash across independent hosts. They do not re-expand. `GET /v1/ingest` cites four laws (cite, don't merge; many indexes, one tip; training is lossy; public verify) plus RE-EXPAND-FROM-ARCHIVE and REHEAL (own last good tip or phoenix-WAIT; forbid vote-to-fix). Growth-ON. FragGate `ledger_tip` and LOCKSET `lockset_sha256` share one tip string on HTML+git. Not a Softwares-tab product. No new MCP tool. Remain-OFF untouched.
