...
- [BAN-SURVIVAL-1.0](designs/BAN-SURVIVAL-1.0.md) — door/path survival: one banned hostname or blocked `/mcp` does not kill clients; named hub `/runtime` + read quarantine + cold tip-hash; never invent a live door
...
- **BAN-SURVIVAL-1.0:** one banned public door (`workers.dev`, `/mcp`, a FragGate path) is a surface death, not last tip gone. Clients try named hub `/runtime` (same FragGate door, service binding), then read surfaces in honest **DEGRADED**, then cold tip-hash. Never invent a live door. Never claim a banned host is LIVE. `GET /survival`. Paper: [BAN-SURVIVAL-1.0](designs/BAN-SURVIVAL-1.0.md).
