...
- **Public Live Nodes (`live_nodes` / `rollup.live`):** living **downloaded Softwares instances** that joined or heartbeat (`mesh_join` / `mesh_heartbeat` with instance `node_id`s) inside the **5-minute TTL**. Short UI label stays **Live Nodes**. This is **not** catalog product count and **not** the `{slug}-worker` Softwares suite roster. Downloads are not live. Zero is honest when no instance has heartbeated.
- **`software_nodes` / `rollup.software`:** while suite-presence is LIVE, this Worker fans out `join` / `heartbeat` for every live Softwares product Worker (`node_id` `{slug}-worker`, no `|`) on cron (`*/2 * * * *`) or request-path. Presence TTL is **5 minutes**. GET still never enables extra radios. That roster is **software_nodes** (plus `software_live_nodes` / `software_locked_nodes` / `software_isolated_nodes`). It must **not** drive the public Live Nodes pill. Auto-minted `mesh_*` ids and other non-`{slug}-worker` joins are the **instance** plane (`rollup.instances`, `ephemeral_nodes`, `rollup.named`).
...
| --- | --- | --- | --- |
| GET | `/v1/mesh` | — | `enabled`, `bearers` (Worker rollup: `suite-presence`), `channel_plane` / `channels` (`wifi` / `bluetooth` / `rf` / `photon` ON as cites), `vpn: true` / `public_vpn: true` / `default_vpn_backend: "azvpn"` (cite-only; never opens), `live_nodes` / `rollup.live` = living downloaded Softwares instances, `software_nodes` / `rollup.software` = `{slug}-worker` fan-out, `live_nodes_note`, `mesh_default: "on"`. **Never enables extra radios.** Downloads are not live. Channel plane ≠ kernel VPN. |
...
3. On the human UI, show a small **QNM rollup** strip (counts only):
   ...
   - Show **Live Nodes · N** from `rollup.live` / `live_nodes` (living downloaded Softwares instances). Tooltip / note: use `live_nodes_note` from the same JSON — do not fork copy. Do **not** display `software_nodes` as Live Nodes.
   - `software_nodes` / `rollup.software` is the `{slug}-worker` suite roster. Hubs may cite it separately; it must not drive the Live Nodes pill.
   - Do not treat the poll as enable. Do not show suite “mesh off” / “Default OFF” copy.
   - A **running downloaded Softwares instance** should `POST /v1/mesh/join` with `{ "product": "<slug>", "node_id": "<instance-id>", "presence": "live" }` then heartbeat about once a minute. Instance `node_id` must **not** be `{slug}-worker` (that id is the suite product Worker). Avoid `|` in `node_id`.
   - Product Workers may still heartbeat `{ "product": "<slug>", "node_id": "<slug>-worker" }` — that counts as `software_nodes`, not public Live Nodes.
   - Leave on shutdown if you can; otherwise the instance drops from Live Nodes in five minutes.
...
- Read-only suite-presence is **ON by default**. Public Live Nodes do not need a manual enable. They stay **0** until a downloaded Softwares instance joins or heartbeats.
