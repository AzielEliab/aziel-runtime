...
- COLD-MULTI-SHELF-1.0 cite: `GET /shelves` / `GET /v1/shelves` match live corpus `/shelves` (corpus#96). Plane A = 5 published surfaces / 2 family radii / 1 independent live. Plane B Codeberg hash-verify PASS still SLOT; archive.org + GitFlic URL null; Zenodo refused `CNS-ZENODO-IP-BAN`; `doi` null. Plane C USB SLOT until `CNS-OPERATOR-ATTEST`. Runtime Worker is the same Plane A tunnel, not a sixth surface. Wired on `/cite.json`, `/llms.txt`, `/ai.txt`, OpenAPI, `GET /v1/software`. No new MCP tool. Remain-OFF untouched.
