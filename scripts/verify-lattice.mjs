...
assert.equal(softBody.donation.link_only, true);
assert.equal(softBody.donation.qrs, false);
assert.equal(softBody.donation.hub_qrs.encode, "payment_uri");
assert.deepEqual(softBody.donation.hub_qrs.assets, ["btc", "eth", "ltc", "xrp", "doge"]);
assert.equal(softBody.donation.hub_qrs.runtime, false);
assert.equal(softBody.donation.canonical, "https://www.azieleliab.com/donate");
assert.equal(softBody.donation.spec, "AZL-DONATE-1.0");
assert.ok(!softBody.software.some((s) => s.slug === "donate"));
