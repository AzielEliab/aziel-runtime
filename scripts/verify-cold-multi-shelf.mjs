...
assert.equal(zenodoShelf.status, "slot");
assert.equal(zenodoShelf.zenodo_live, false);
...
assert.equal(cite.shelves.plane_b.zenodo.status, "slot");
assert.equal(cite.shelves.plane_b.zenodo.zenodo_live, false);
...
assert.match(llms, /CNS-ZENODO-NOT-LIVE|zenodo_live:false|Zenodo tip-pack stays SLOT/);
assert.doesNotMatch(llms, /CNS-ZENODO-IP-BAN|Operator IP banned/);
...
assert.match(skill, /CNS-ZENODO-NOT-LIVE|zenodo_live:false|Zenodo tip-pack stays SLOT/);
assert.doesNotMatch(skill, /CNS-ZENODO-IP-BAN|Operator IP banned/);
