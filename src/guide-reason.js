...
function flowAnswer(intent, cards) {
  ...
  if (intent === "fraggate") {
    return {
      topic: "fraggate",
      answer:
        "Click the domain tab for the Software, then Run on its card. Call the Softwares tool. The door runs first. ChainLock stamps when the call needs a ledger. confirm=true writes. dry_run=true previews and writes nothing. background=true returns Running until a receipt hash exists. fraggate_list, fraggate_describe, fraggate_call, and chainlock tools stay for diagnostics. Seal on the interface desk and the AZBot Seal button still need the confirm box. This guide does not dispatch.",
      actions: [
