...
assert.match(skillText, /Amazon Q \/ Amazonbot tooling/);
assert.match(skillText, /DuckAssist \/ DuckDuckGo AI/);
assert.match(skillText, /plus other MCP\/OpenAPI-capable assistants/);
assert.doesNotMatch(skillText, /How Grok \/ ChatGPT \/ Venice pull \+ call/);
assert.doesNotMatch(skillText, /Grok\/ChatGPT\/Venice Actions that \*\*exec\*\*/);
...
assert.match(skillText, /REMAIN-OFF-BY-DESIGN-2026-09-10/);
assert.match(skillText, /constitutional OFF set/);
assert.match(skillText, /AZL-DONATE-1\.0/);
assert.match(skillText, /live-with-local-destructive-boundary/);
assert.doesNotMatch(skillText, /EmbryoLock is STUB at FEATURE-STATE ingest/);
...
assert.match(skillText, /\/v1\/uses/);
...
assert.equal(manifest.host, `${origin}/`);
assert.equal(manifest.primary_host, "https://glama.ai/mcp/servers/AzielEliab/aziel-runtime");
assert.equal(manifest.homepage, "https://glama.ai/mcp/servers/AzielEliab/aziel-runtime");
assert.equal(manifest.library_mirror, "https://www.azielcorpuslibrary.net/runtime");
assert.equal(manifest.library_front_door, undefined);
...
assert.ok(manifest.endpoints.software.includes("/v1/software"));
assert.ok(manifest.endpoints.update_check.includes("/v1/update/check"));
assert.ok(manifest.endpoints.uses.includes("/v1/uses"));
assert.ok(manifest.endpoints.stats_rollups.includes("/v1/stats-rollups"));
assert.ok(manifest.uses.includes("/v1/uses"));
...
assert.ok(pull.install_inspect && pull.install_inspect.url.endsWith("/install.sh"));
...
assert.equal(health.uses, "/v1/uses");
assert.equal(health.primary_host, "https://glama.ai/mcp/servers/AzielEliab/aziel-runtime");
assert.equal(health.homepage, "https://glama.ai/mcp/servers/AzielEliab/aziel-runtime");
assert.equal(health.host, `${origin}/`);
assert.equal(health.library_mirror, "https://www.azielcorpuslibrary.net/runtime");
assert.equal(health.library_front_door, undefined);
assert.equal(health.stats, "/v1/stats");
assert.equal(health.stats_rollups, "/v1/stats-rollups");
assert.equal(health.uses_total, undefined);
...
assert.match(homeHtml, /Rose-star — Aziel Eliab/);
assert.match(homeHtml, /src="\/sigil\.png"/);
assert.doesNotMatch(homeHtml, /everbloom/i);
...
assert.equal(openapi.info.title, "Aziel Runtime");
...
assert.ok(tools.includes("Softwares"));
assert.ok(!tools.includes("runtime_software"));
...
assert.ok(tools.length <= 40, `tools/list ${tools.length} > 40`);
assert.ok(tools.includes("memory_recall"));
assert.ok(tools.includes("memory_observe"));
assert.ok(tools.includes("memory_calibrate"));
assert.ok(tools.includes("mesh_status"));
assert.ok(tools.includes("chainlock_append"));
assert.ok(tools.includes("chainlock_seal"));
assert.ok(tools.includes("mesh_join"));
assert.ok(tools.includes("mesh_nodes"));
...
assert.match(localSkill, /Claude \(Anthropic Desktop \/ custom tools\)/);
assert.match(localSkill, /Cursor \(MCP\)/);
assert.match(localSkill, /Cohere/);
assert.doesNotMatch(localSkill, /How Grok \/ ChatGPT \/ Venice/);
const localManifest = runtimeManifest(origin, PRODUCTS);
assert.ok(localManifest.compatible_ai_clients.includes("Claude (Anthropic Desktop / custom tools)"));
assert.match(localManifest.crawler_allow, /GPTBot\/ChatGPT/);
assert.match(localManifest.crawler_allow, /Baiduspider\*/);
