...
- **Softwares tab:** Plain → Gate → Lock (Clock ≠ Lock). Hub UI is heading → list only. `one_line` is designed action; `description` is Use X to … It exists so …. No THIS-IS / THIS-IS-NOT template, never-invent bans, or verified-status marketing. No “runtime X.Y FragGate” mash.
...
| 4 | **Catalog `one_line` / `description`** | Designed action, then Use X to … It exists so …. No THIS-IS / THIS-IS-NOT template, never-invent bans, or verified-status marketing. No marketing mash, no runtime-version + FragGate mash. |
...
| — | `whitestone` | live Worker-only placement (1.6.0 Case Mode). Softwares-tab Plain. Counted zip `whitestone-download-tracker` (ARK-style worker_home). Live web app `https://whitestone.vibelock.workers.dev/`. Historical as-of + Case Mode (suppression axes, TrajectoryLock-lite, export, confidence ≤75%). Dual-surface AI discovery via runtime llms/ai/cite/who-is + product `GET /v1/software`. No FragGate engine / no invented op. Not FragGate kernel. Not a lawyer / not legal advice. Session-only. Isolation 33 unchanged. See `docs/2.0/WHITESTONE-PLACEMENT.md`. |
...
| **forgereceipts** | `verify`, `import_export`, `doctor` | `court`, `legal_advice`, `odyssey`, `file_store` | Recompute receipt hash. Axes include request_id, attempt_n, parent_receipt_id, correlation_id inside the canonical hash. Client-held JSON import/export. Not legal advice. Neighbor: TemporalLock (older receipt shape). |
...
| **spectrallock** | `targets`, `verify`, `doctor`, `pigment`, `restore-pigment` | `spectrometer`, `forensic`, `invent_mark` | Overlay metadata hash, not a forensic claim. Wheel paint is separate from the spectral triad. Overlay accepts `inject` true\|false (paint ≠ pigment). Reports `tazel_inband_pct` / `vyrn_inband_pct`. Restore lost pigment is LIVE on FragGate `pigment` / `restore-pigment` (`listPigment` / `pigmentFromB64`) and refuses `SL-PIGMENT-GONE` when evidence is gone. AMOE is not a live product. Vendored overlay still carries leftover-bytes unredact, incremental PDF `revision_graph`, universal recover (`SL-RECOVER-*`; 7z/HEIC/HEIF SLOT), and handwriting — those stay **off** the FragGate door. Product Worker doors stay `/v1/unredact`, `/v1/recover`, `/v1/handwriting`. Never OCR-from-black-box. No hosted media store. Neighbors: 4DMap Γ, TrajectoryLock, Aziel Corpus. |
...
| miragegrid | assign, route, circuit, verify-receipt, nodes, bridge, shuffle, mesh | assign, verify-receipt, nodes, bridge, shuffle | Cap-7 bridge cite + ping→land shuffle (inherit designs only; resolves_to_hub false; hosted update SLOT). mesh/hop stay stub |
