...
| Y6 Live Nodes rollup | **CLOSED (1.9.1 split)** — then `live_nodes` was Softwares `{slug}-worker` only (`mesh_*` = `ephemeral_nodes`). **OPERATOR LAW (later):** public `live_nodes` was mesh size (active + inactive, not isolated). **OPERATOR OVERRIDE:** public `live_nodes` counted human mesh users plus cited human uses (`USES`). **OPERATOR LOCK 2026-09-21:** `nodes` = `human_mesh_users` + `human_uses`; `live_nodes` = `human_mesh_users` (presence). **OPERATOR LOCK 2026-09-21 addendum:** `live_nodes` = `human_mesh_users` + `site_live_viewers`. `{slug}-worker` roster is `software_nodes`. Downloaded instances stay `instance_nodes`. |
