...
assert.match(product.oneLine, /Verify hash continuity/i);
assert.doesNotMatch(product.oneLine, /THIS IS:|THIS IS NOT:/i);
assert.match(product.description, /never hosts files|without hosting payloads/i);
assert.match(product.banner, /FragGate/);
assert.match(product.banner, /Separate software/);
...
assert.equal(status.pair_state, "unpaired");
assert.equal(status.functional_order, true);
assert.equal(status.pairing, "order/token");
assert.equal(status.pairing_kind, "functional-order");
assert.equal(status.peer, "azbrowser");
...
assert.equal(status.pair.tunnel, false);
...
assert.equal(doorPair.result.pair_state, "unpaired");
assert.equal(doorPair.result.vpn, false);
assert.equal(doorPair.result.tunnel, false);
assert.equal(doorPair.result.pairing, "order/token");

const doorPairAlias = await post("/v1/fraggate/call", { slug: "aznet", op: "pair", payload: {} });
assert.equal(doorPairAlias.ok, true);
assert.equal(doorPairAlias.code, "FG-OK");
assert.equal(doorPairAlias.op, "pair");
assert.equal(doorPairAlias.canonical_op, "pair_status");
assert.equal(doorPairAlias.result.pair_state, "unpaired");
assert.equal(doorPairAlias.result.vpn, false);
