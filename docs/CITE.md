...
- **Plane B** SLOT — Codeberg https://codeberg.org/AzielEliab/aziel-lockset-tip hash-verify PASS (pack `b549362c0736ddb54ddc488812327c464e0da1167281f92fd1a4263eedf5df37`) still SLOT. archive.org PASS https://archive.org/details/aziel-lockset-tip + https://archive.org/details/aziel-lockset-tip_202609 (same blast_radius `archive-org`; same pack) still SLOT. Framagit URL **null** (third ALL-TARGETS). GitFlic refused `CNS-GITFLIC-EMAIL`. GitLab refused `CNS-GITLAB-CF-LOOP`. LIVE only after all three (`CNS-PLANE-B-ALL-TARGETS`). Zenodo refused `CNS-ZENODO-IP-BAN`. `doi` null — never invent.
