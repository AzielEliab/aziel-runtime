...
**Runtime 1.9.2** · Author: **Aziel Eliab** only.
...
| aziel-corpus | search, example, review, score, verify-backfill, verify-geo, document-chain, import_export, health, skill, doctor | jeeves, media-run (proxy); transcribe / ocr are Workers-AI-gated (native when `env.AI` is bound — wrangler binds `AI`); live D1 search queries production `records` when `CORPUS_D1` is bound (wrangler binds `aziel-digital-library`) |
