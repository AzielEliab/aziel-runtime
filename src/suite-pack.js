 ...
 *
 * Do not invent binary contents. Do not claim .
...
Start here: call the Softwares tool. The door runs first. ChainLock, TemporalLock, and ForgeReceipts stamp when the call needs a ledger. confirm=true writes. dry_run=true previews. background=true stays Running until a receipt hash exists. tools/list stays 36.
...
export function buildSuitePack({ origin, products, registry, extra = {} } = {}) {
  ...
  return {
    ...
    note:
      "One-click suite pack for humans and machines. Packs what ships in-process. Worker wasm, WireGuard/OpenVPN, and qnm-node stay SLOT/CITE. FragGate is THE exec door. Identity Aziel Eliab only. Never .",
  };
}
