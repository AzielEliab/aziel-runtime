...
- OPERATOR LAW honesty (not a new public MCP tool): public `live_nodes` / `rollup.live` count living downloaded Softwares instances (join/heartbeat; TTL 5 min). `{slug}-worker` suite fan-out is `software_nodes` / `rollup.software`. Downloads are not live. Zero is honest. Hubs read runtime SoT (`live_nodes` + `live_nodes_note`). See [`docs/NODE_MESH.md`](../NODE_MESH.md).
...
- Additive FoldLock corpus **tip** + About Aziel (not a new public MCP tool): `AZCL-FOLD-TIP-1.0` ships a hash-verified in-process archive of the library index cite, bundled sample-MASTER key artifacts, and published About Aziel. `full_library_in_process` stays false. FragGate `foldlock/pack-verify` and `aziel-corpus/tip-pack` (2.x additive engine ops). Human dashboard + `/about` + product control panel. Hash mismatch refuses. Cap-7 / shelves untouched. No invented DOI / Framagit / Glama UUID / fielded-100. See [`docs/corpus-fold-pack.md`](../corpus-fold-pack.md).
