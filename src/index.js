/**
 * aziel-runtime 1.7.8 — EmbryoLock true in-process engine (live-with-local-destructive-boundary; worker_home embryolock-download-tracker) + 1.7.7 AZCoherence (AZC-0.1) true in-process FragGate Softwares engine + 1.7.6 4DMap LIVE_OPS synced with product 0.2.0 + 1.7.5 Softwares capability wave 1 (decisiongate / forgereceipts / temporallock / staticclock / chronolock / trajectorylock / spectrallock) + 1.7.4 4DMap inspection-frame capability bump (frame_status/axis_describe/walk_trace/card_export/card_import/verify_chain/neighbor_cite; not an extra door) + 1.7.3 audit WARN copy/hint (exist.mcp → tools/list; public FragGate call; catalog count_note; 4DMap not an extra door) + 1.7.2 GET /v1/azpipe/arch MASTER-33 cite + AKM-TRIAD-1.0 + MASTER-33 (FragGate single door) + locked suite heritage (SUITE-PIPE-1.6.15) + 4DMap (4DM-WP-1.0) + QNM-BUILD-1.0 suite rollup (default OFF; not a login mesh) + live software catalog + client update check + GitHub auto-deploy + FragGate door + UI-op aliases + AZChat name-only stub + AZHub + AZInterface + AZNet + AZBrowser + AZMail + PeaceLock + KV-backed API use trackers.
 *
 ...
 * GET  /v1/runtime.json       machine manifest: role=engine-runtime (1.7.8), door=fraggate
