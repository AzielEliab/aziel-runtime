...
export function glamaInstallCite(workerVersion = "2.0.0-rc1") {
  return {
    ...
    first_call: "call the Softwares tool — the door runs first; ChainLock, TemporalLock, and ForgeReceipts stamp when the call needs a ledger",
    first_call_examples: [
...
export function llmsWhatThisIsBlock(calling = null) {
  ...
  return [
    ...
    "",
    "Also:",
    "Agents: call the Softwares tool. The door runs first (POST /mcp). ChainLock, TemporalLock, and ForgeReceipts stamp when the call needs a ledger. Diagnostics: fraggate_list → fraggate_describe → fraggate_call.",
    ...
    "First call: the Softwares tool. The door runs first. ChainLock, TemporalLock, and ForgeReceipts stamp when the call needs a ledger. Example: decisiongate_check with dry_run=true, or foldlock fold-preview.",
    "",
