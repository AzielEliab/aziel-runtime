...
- [BAN-SURVIVAL-1.0](designs/BAN-SURVIVAL-1.0.md) — live multi-front door survival: one banned hostname or blocked `/mcp` does not kill clients; workers.dev + custom-domain `/runtime` + path quarantine; LIVE doors only; never invent a live door; never claim shelves saved you
...
- **BAN-SURVIVAL-1.0:** one banned public door (`workers.dev`, `/mcp`, a FragGate path) is a surface death, not the runtime gone. Clients try the next LIVE custom-domain hub `/runtime` (same FragGate door, service binding), then remaining LIVE exec/read paths. Door list = LIVE only. Never invent a live door. Never claim a banned host is LIVE. Never claim shelves saved you. `GET /survival`. Paper: [BAN-SURVIVAL-1.0](designs/BAN-SURVIVAL-1.0.md).
