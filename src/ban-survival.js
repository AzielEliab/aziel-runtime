...
import { SPORE, reColdStoreCite, sporeCite, sporeLlmsBlock, sporeSkillMarkdown, survivalStackCite } from "./spore.js";
...
export const LIVE_NODE_API = Object.freeze({
  ...
  note:
    "QNM nodes count human mesh users plus cited human uses (USES). live_nodes count human mesh users plus site_live_viewers. software_nodes is the Softwares roster and never an API. Roster nodes do not publish exec URLs. Product Workers proxy /v1/mesh/* only — not /mcp. No submesh/subpipe exec hop. Open proxy to random nodes is refused.",
...
export function survivalDoc(origin, env) {
  ...
  return {
    ...
    survival_stack: survivalStackCite(),
...
export function survivalLlmsBlock(origin, env) {
  ...
  return [
    ...
    "Cap-7 factory is MirageGrid-only .az duplication + shift (StaticLock; catalog product StaticClock, slug staticclock; MirageGrid cloak + AZVPN). Standard internet does not reach Cap-7. Internet reaches AZ domains via the four hub HTTPS links. public_icann and resolves_to_hub are that AZ path. Real duplications: azgrid, azcloak, azvault, azshift. False sites: azbooth, azflag, azstandby. radio_phy false. AZNet never hosts payloads. icann_tld_az false.",
