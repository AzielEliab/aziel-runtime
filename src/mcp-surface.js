...
export function mcpInitializeInstructions() {
  return (
    ...
    "Execute only through fraggate_call (CallEnvelope → FragGate → SweepGate → ChainLock-IN → DecisionGATE → AZPIPE → Domain Doors → TemporalLock → StaticClock → ChainLock-OUT → Response/Receipt). " +
    ...
    "1.6.15 locks the suite hop order (SUITE-PIPE-1.6.15): PUBLIC/UI/Agents → FragGate → SweepGate → ChainLock-IN → DecisionGATE → AZPIPE → Domain Doors (4DMap inspection) → TemporalLock → StaticClock → ChainLock-OUT → Response/Receipt. LambGate is not a hop. " +
...
export function runtimeHelperTools() {
  return [
    ...
    },
    {
      ...
      description:
        "Step 3 of the agent pipeline (after list + describe). Locked path: CallEnvelope → FragGate → SweepGate → ChainLock-IN → DecisionGATE → AZPIPE → Domain Doors (4DMap inspection) → TemporalLock → StaticClock → ChainLock-OUT → Response/Receipt. DecisionGATE after ChainLock-IN, before domain exec. Pass slug (or name) plus a public allowlisted op. Optional claim (DecisionGATE proposal). Default exec path. Unknown names refuse FG-HALLUC-TOOL. UI aliases (list_modules, place, genesis_boot, hold, airlock, home, classify, doctor, pair) forward to catalog ops. AZHub LIVE_OPS (region_list, place_module, remove_module, tether_declare, tether_cut, tether_list, blank_key_status, list_modules, place, health, skill) and AZInterface LIVE_OPS (genesis_status, site_state_get, site_state_set, integrity_check, witness_list, page_cycle_status, genesis_boot, hold, health, skill) and AZBrowser LIVE_OPS (ethical_search, lamb_lens_search, navigate, airlock_ingest, airlock, home, tab_open, tab_list, receipt_list, verify, receipt_verify, health, skill) map here — same backend as the Worker UI buttons. Sibling software under one FragGate door.",
