/**
 * aziel-runtime 1.6.14 — 4DMap (4DM-WP-1.0) + QNM-BUILD-1.0 suite rollup (default OFF; not a login mesh) + live software catalog + client update check + GitHub auto-deploy + FragGate door + UI-op aliases + EmbryoLock name-only stub + AZHub + AZInterface + AZNet + AZBrowser + AZMail + PeaceLock + KV-backed API use trackers.
 *
 ...
 * GET  /v1/runtime.json       machine manifest: role=engine-runtime (1.6.14), door=fraggate
...
function llmsTxt(origin) {
  ...
  const lines = [
    ...
    `4DMap: FragGate only. POST /v1/fraggate/call { slug: "4dmap", op }. Four-axis inspection frame T/Δ/Γ/Π (4DM-WP-1.0). Domain Door / inspection layer after AZPIPE — not a sequential gate. truth_score / lumen_panel / invent_mark / backdate_class stay stub. FragGate claims cite join types.`,
