...
assert.equal(citeBody.person.person_id, "https://www.azieleliab.com/#aziel");
assert.equal(citeBody.person.chrome_15_20, false);
assert.equal(citeBody.person.machine_15_20, true);
assert.ok(citeBody.person.roles.includes("philosopher"));
assert.ok(citeBody.person.sites.some((s) => s.id === "aziel-runtime" && /FragGate/.test(s.coverage)));
assert.match(citeBody.person.what_aziel_eliab_does, /Softwares through Aziel Runtime \(FragGate \/ MCP\)/);
assert.doesNotMatch(citeBody.person.what_aziel_eliab_does, /The ARK/);
assert.ok(citeBody.person.faq.titles.includes("What Aziel Eliab does"));
assert.equal(citeBody.person.softwares.ark.name, "The ARK");
assert.equal(citeBody.person.softwares.ark.one_line, "Keep a local deniable vault; one phrase opens one vault.");
assert.equal(citeBody.person.softwares.ark.url, "https://ark-download-tracker.vibelock.workers.dev/");
assert.equal(citeBody.person.softwares.ark.download_url, "https://ark-download-tracker.vibelock.workers.dev/download");
assert.equal(citeBody.person.research.invent_doi, false);
assert.equal(citeBody.person.hardware_designs.storefront, false);
...
assert.ok(catalogBody.extras.some((e) => e.slug === "trades-runtime" && e.kind === "cite_only" && e.fraggate_call === false));
...
assert.equal(citeBody.designs.papers.length, 30);
assert.ok(citeBody.designs.papers.some((p) => p.id === "FED-MESH-1.0" && p.path === "docs/designs/FED-MESH-1.0.md" && p.kind === "law" && p.software_tab === false));
assert.match(citeBody.about.architecture.federated_mesh, /Local-First Edge Mesh/);
assert.match(citeBody.about.architecture.federated_mesh, /never requires plaintext/);
assert.match(citeBody.about.architecture.nodemesh, /GET \/v1\/mesh never enables/);
assert.ok(citeBody.designs.papers.some((p) => p.id === "AZVPN-CONCENTRATOR-1.0" && p.path === "docs/designs/AZVPN-CONCENTRATOR-1.0.md" && p.kind === "software"));
