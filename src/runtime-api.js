...
export const RUNTIME_VERSION = "1.7.5";
...
export const VERSION_HISTORY = [
  { version: "1.7.5", status: "current", note: "Softwares capability wave 1: enrich health/skill (axes/neighbors/ops table) and add domain-true doctor/verify/import_export (or window/gates/schema/targets) on decisiongate, forgereceipts, temporallock, staticclock, chronolock, trajectorylock, spectrallock. Catalog one_line capability-forward. LIVE_OPS + catalog ops + engine *_OPS agree. Suite checklist at docs/audit/SUITE-CAPABILITY-CHECKLIST.md. Remain-Off-by-Design (FEATURE-STATE-2026-09-10): mesh, ARK wipe, VeilLock inject, AZ-OS shell, 4DMap truth_score/invent_mark/backdate, and public rollback stay refused. AKM-TRIAD-1.0 stays LIVE fabric (POST /v1/memory/*, MCP memory_*; not Softwares-tab). 4DMap 1.7.4 inspection-frame ops stay. EmbryoLock/AZChat stay stubs. Identity Aziel Eliab only." },
...
description: >-
  ...
  install, and counted /download stay complete human software. 1.7.5 is Softwares
...
**1.7.5 = Softwares capability wave 1.** Rich health/skill plus domain-true doctor/verify/import_export (or window/gates/schema/targets) on DecisionGATE, ForgeReceipts, TemporalLock, StaticClock, ChronoLock, TrajectoryLock, SpectralLock. Catalog \`one_line\` is capability-forward. LIVE_OPS matches engine ops. Checklist: \`docs/audit/SUITE-CAPABILITY-CHECKLIST.md\`. Remain-Off-by-Design (\`FEATURE-STATE-2026-09-10\`): mesh, ARK wipe, VeilLock inject, AZ-OS shell, 4DMap truth_score/invent_mark/backdate, and public rollback stay refused. AKM-TRIAD-1.0 stays LIVE fabric (not Softwares-tab). 4DMap 1.7.4 inspection-frame ops stay. Mesh stays default-off.
  ...
  return {
    ...
    "/v1/skill": {
      get: {
        ...
        summary: "Skill markdown: 1.7.5 Softwares capability wave 1 (decisiongate / forgereceipts / temporallock / staticclock / chronolock / trajectorylock / spectrallock). 1.7.4 enhances 4DMap LIVE_OPS (frame_status/axis_describe/walk_trace/card_export/card_import/verify_chain/neighbor_cite). 1.7.3 aligns audit WARN copy (exist.mcp → tools/list; public FragGate call; catalog count_note). 1.7.2 adds GET /v1/azpipe/arch (MASTER-33 cite). 1.7.1 adds AKM-TRIAD-1.0. 1.7.0 locks MASTER-33 (FragGate single door). 1.6.15 locked SUITE-PIPE. 1.6.14 adds 4DMap (4DM-WP-1.0). 1.6.13 aligns the QNM-BUILD-1.0 suite rollup. 1.6.12 adds GET /v1/software + client update check. Honest about 1.1.0 through 1.7.5.",
        tags: ["runtime"],
