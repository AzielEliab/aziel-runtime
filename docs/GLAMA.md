...
| File | Role |
|------|------|
| [`glama.json`](../glama.json) | Claim file. Schema requires `maintainers` (GitHub username `AzielEliab`). Listing `name`, `version` (`2.0.0-rc1`), `description` (NodeMesh'd MCP Softwares suite for digital forensics/auditing; `1.6.2` is superseded heritage; current server `2.0.0-rc1`), `keywords` (includes mcp / openapi / fraggate / digital-forensics), and `categories` are additional properties. |
