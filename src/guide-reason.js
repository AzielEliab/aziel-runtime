...
function flowAnswer(intent, cards) {
  ...
  if (intent === "fraggate") {
    return {
      topic: "fraggate",
      answer:
        "Click the domain tab for the Software, then Run on its card. The door runs first, then the tool. confirm=true writes. dry_run=true previews and writes nothing. background=true returns Running until a receipt hash exists. fraggate_list, fraggate_describe, and fraggate_call stay for diagnostics. Seal on the interface desk and the AZBot Seal button still need the confirm box. This guide does not dispatch.",
      actions: [
