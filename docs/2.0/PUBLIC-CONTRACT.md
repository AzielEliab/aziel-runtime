...
- Mesh: `mesh_status`, `mesh_enable` (**`bearer` required**), `mesh_disable` (**refuses `MESH-DISABLE-REFUSED`** — public suite disable refused), `mesh_join` (**`product` required**; optional `node_id` exactly 8–80 `[a-z0-9._-]`; `presence` `live`\|`locked`\|`isolated`; **strict 5-minute TTL**; radios off → **`MESH-OFF`**), `mesh_heartbeat` (**`node_id` required**; refreshes TTL), `mesh_leave` (**`node_id` required**), `mesh_nodes`, `mesh_broadcast` (**`sha256` required**)
...
- Preferred / claimed `protocolVersion`: `"2025-11-25"` (streamable HTTP session + protocol headers). Still accepts `"2025-06-18"` and `"2025-03-26"` (compatibility floor — not echoed as the claimed version unless `require_legacy_protocol=true`). `latest` / `validator_protocol_version` map to preferred. Invalid `MCP-Protocol-Version` is HTTP 400.
- Mutating MCP tools (`fraggate_call`, `runtime_run`, session mutate, `decisiongate_check`, mesh join/leave/enable/heartbeat/broadcast, chainlock append/seal, memory observe/resolve/calibrate) document `confirm` / `dry_run` and set `annotations.requiresConfirmation`. Those flags are **not** listed in `inputSchema.required` (connector refresh must not invent a new required arg). `tools/call` still requires `confirm=true` or `dry_run=true` (`MCP-CONFIRM-REQUIRED` otherwise). HTTP `POST /v1/fraggate/call` is unchanged. FragGate stays THE single door.
...
| GET | `/v1/mesh/az-generator` | Cap-7 semantic-bridge cite (MirageGrid `.az` duplication; standard internet does not reach Cap-7; `design_of: hub_designs`; Cap-7 `resolves_to_hub: false`; AZ domains resolve via hub HTTPS; 3 of 7 false sites) |
...
Additive `durability` on `GET /v1/ready`, `GET /v1/fraggate` (+ list), and `GET /v1/mesh` labels the ephemeral FragGate ledger window (cap 64) versus ChainLock / SESSION durable commits when those Durable Objects are bound. **MemoryStore is never durable.** AKM Belief List isolate index is a derived cache; recollection authority is ChainLock learn (rebuildable). HTTP `dry_run` does not write.

F03 door gates: FragGate HTTP + MCP share per-IP rate limits (`RATE_LIMIT` 429). Direct `POST /v1/mesh/join|heartbeat|leave|broadcast|site-presence` is kind `mesh_mutate` (30/min). Direct `POST /v1/memory/*` is kind `memory_mutate` (60/min). Body over 256 KiB is `BODY_TOO_LARGE` (413). JSON deeper than 12 / wider than 4096 nodes is `BODY_TOO_DEEP` (400). Request budget default 25s is `REQUEST_DEADLINE` (408). `RATE` Durable Object is the Worker-feasible distributed quota when bound; isolate otherwise (`enforcement` is labeled). Mesh join stays presence-only (not a login mesh). `/p/{slug}/{op}` forwards documented tracker/catalog ops only (`PROXY-OP-REFUSED` for factory `/mcp`).
...
- No invented OAuth IdP / Zenodo DOI / Framagit URL / Glama UUID / .
