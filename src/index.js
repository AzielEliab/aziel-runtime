/**
 * aziel-runtime 1.7.1 — AKM-TRIAD-1.0 + MASTER-33 (FragGate single door) + locked suite heritage (SUITE-PIPE-1.6.15) + 4DMap (4DM-WP-1.0) + QNM-BUILD-1.0 suite rollup (default OFF; not a login mesh) + live software catalog + client update check + GitHub auto-deploy + FragGate door + UI-op aliases + EmbryoLock name-only stub + AZHub + AZInterface + AZNet + AZBrowser + AZMail + PeaceLock + KV-backed API use trackers.
 *
 ...
 * GET  /v1/runtime.json       machine manifest: role=engine-runtime (1.7.1), door=fraggate
...
function llmsTxt(origin) {
  ...
  const lines = [
    ...
    `4DMap: FragGate only. POST /v1/fraggate/call { slug: "4dmap", op }. Four-axis inspection frame T/Δ/Γ/Π (4DM-WP-1.0). Research-domain inspection frame inside Internal Domain Layer after AZPIPE — not a sequential gate and not an extra door. Cited on the locked MASTER-33 pipeline. truth_score / lumen_panel / invent_mark / backdate_class stay stub. FragGate claims cite join types.`,
    `Locked MASTER-33 pipeline: ${LOCKED_STRIP}. FragGate is THE single door. Lamb Lens is fabric after FragGate. Domains are isolation labels, not doors. LambGate is not a hop. FoldLock fld3-wire is internal to AZPIPE. Illegal reorder is refused. No rollback.`,
