...
- Nine QNM / Node Mesh laws are hard-true on `GET /v1/mesh` (and status): machine fields plus published refuse codes (split-wires, cold-copy, REHEAL refuse, phoenix local-only, die-with-pull does not restore godlock.uk, no neighbor heal, no Node Gate, no auto-heal, not an anonymity network). Close test `scripts/verify-mesh-nine-laws.mjs`. No new MCP tool. Remain-OFF untouched.
