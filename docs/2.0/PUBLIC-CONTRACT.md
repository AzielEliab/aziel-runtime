...
- Mesh: `mesh_status`, `mesh_enable` (**`bearer` required**), `mesh_disable` (**refuses `MESH-DISABLE-REFUSED`** — public suite disable refused), `mesh_join` (**`product` required**), `mesh_heartbeat` (**`node_id` required**), `mesh_leave` (**`node_id` required**), `mesh_nodes`, `mesh_broadcast` (**`sha256` required**)
