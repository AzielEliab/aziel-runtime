#!/usr/bin/env python3
"""Render docs/designs/NO-LIE-NO-REWRITE-1.0.md to a simple text PDF. Author: Aziel Eliab only."""
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
SRC = ROOT / "docs/designs/NO-LIE-NO-REWRITE-1.0.md"
OUT = ROOT / "docs/designs/NO-LIE-NO-REWRITE-1.0.pdf"
...
def page_stream(lines, page_no, pages):
    ...
    cmds.append(
        "(%s) Tj"
        % escape("NO-LIE-NO-REWRITE-1.0  ·  Aziel Eliab only  ·  page %d / %d" % (page_no, pages))
    )
    cmds.append("ET")
    return "\n".join(cmds)
