...
const PRODUCTS_RAW = [
  ...
  },
  {
    ...
    ops: [
      ...
      { op: "modes", method: "GET", summary: "List live overlay modes (zero, tazel, vyrn, uv, rosetta, zen, chaos, balance, candle, indent, lemon). Wheel paint is separate from the spectral triad. Reports live pigment / restore-pigment plus leftover-bytes, revision-graph, universal recover, and handwriting metadata. Unredact / recover / handwriting are not FragGate door ops." },
      { op: "targets", method: "GET", summary: "List ink/page targets. Same targets as Aziel Corpus Library OCR." },
      { op: "overlay", method: "POST", summary: "Simplified overlay preview. PNG b64 in, longest side capped at 256 px. Accepts inject true|false (ON = wheel-paint membership, not recovered pigment; OFF is gray of the same gate; zero ignores the switch). Reports tazel_inband_pct and vyrn_inband_pct. Not the pigment door and not the full Python pipeline." },
      { op: "pigment", method: "POST", summary: "Restore lost pigment from a PNG where faded signal remains (listPigment / pigmentFromB64). Ops restore|estimate|refuse. Refuses SL-PIGMENT-GONE when the signal is gone. pigment_recovery is true on this path. 256 px preview." },
      { op: "restore-pigment", method: "POST", summary: "Alias of pigment. Restore lost pigment. Same engine as pigmentFromB64." },
      { op: "verify", method: "POST", summary: "Recompute overlay metadata hash (mode/target/geometry). Not forensic. Not OCR-from-black-box." },
      { op: "doctor", method: "GET", summary: "Richer liveness: 256px preview, not a spectrometer. Wheel paint is separate from the triad. Restore lost pigment is LIVE. AMOE is not a live product. Inject ON is not pigment recovery. Unredact / recover / handwriting are not FragGate door ops." },
    ],
    example: { mode: "rosetta", inject: true, b64: "<png-base64>" },
    banner: "Hosted overlay is a 256px preview. SpectralLock 0.3.1. Wheel paint is a membership-tint plane separate from the spectral triad (Tazel #1EC9A5, Vyrn #C00066, Zero #6F6485). Inject ON paints wheel membership, not recovered pigment. Restore lost pigment is LIVE on FragGate ops pigment and restore-pigment and refuses SL-PIGMENT-GONE when the faded signal is gone. AMOE is not a live product. Leftover-bytes recover, universal recover, and handwriting stay on the product Worker and are not FragGate door ops. UV is not a lamp. Balance/lemon/indent never invent marks.",
  },
  {
    slug: "azbot",
    name: "AZBot",
  ...
  },
  {
    ...
    ops: [
      ...
      { op: "mesh_post", method: "POST", summary: "Post to the AZMail anonymous mail ring. Refuses when the mail ring is off (product-local default). Strips identity keys." },
      ...
      { op: "mesh_enable", method: "POST", summary: "Turn the AZMail anonymous mail ring on. Rate-limited. Mail ring default is off." },
      { op: "mesh_disable", method: "POST", summary: "Turns the AZMail anonymous mail ring off. Product-local — not the QNM suite." },
  ...
  },
  {
    ...
    ops: [
      { op: "doctor", method: "GET", summary: "Richer liveness: hash-commitment attest, refuse list. Does not increment download KV." },
      { op: "commit", method: "POST", summary: "SHA-256 commitment of salt+witness. Witness is not returned. Not a SNARK." },
      { op: "attest", method: "POST", summary: "Bind a public statement to a commitment. Refuses a posted witness." },
      { op: "open", method: "POST", summary: "Check a commitment opening the caller already holds. Reveals; not a ZK proof." },
      { op: "verify", method: "POST", summary: "Recompute a public attest receipt hash. Does not recover a witness." },
      { op: "limitation", method: "GET", summary: "Honesty labels: REAL hash commitment; SLOT for Groth16/SNARK." },
    ],
    example: { statement: "receipt bound without publishing the witness", commitment: "0".repeat(64) },
    banner:
  ...
  },
  {
    ...
    github: "https://github.com/AzielEliab/aziel-runtime",
    ops: [
      { op: "doctor", method: "GET", summary: "Richer liveness: posted-opinion tally, refuse list. Does not increment download KV." },
      { op: "tally", method: "POST", summary: "Majority over caller-supplied verdicts. Does not call models." },
      { op: "agree", method: "POST", summary: "Token Jaccard over posted opinion texts. HEURISTIC. Not live inference." },
      { op: "limitation", method: "GET", summary: "Honesty labels: HEURISTIC tally; SLOT for live multi-model calls." },
    ],
    example: {
  ...
  },
  {
    ...
    ops: [
      ...
      { op: "limitation", method: "GET", summary: "Honesty labels: REAL door classification; SLOT for invent_completeness." },
    ],
    example: { id: "fg-halluc" },
    banner:
      "ToolBench (TOOLBENCH-0.1): synthetic FragGate / Sentinel / TDQS refuse playground. Self-test ≠ third-party lab. Not . In-runtime placement. FragGate only. Author Aziel Eliab.",
  },
  {
    slug: "azvpn",
    name: "AZVPN",
...
function productUrls(product, origin) {
  ...
  return {
    ...
    skill: host ? host + "/v1/skill" : `${base}/v1/skill`,
    openapi: host ? host + "/openapi.json" : `${base}/openapi.json`,
    stats: host ? host + "/stats" : `${base}/v1/uses`,
    in_runtime: !host,
...
function workerOnlyCiteRecord(spec, origin) {
  ...
  return {
    ...
    cite: null,
    llms: null,
    catalog_card: spec.web_app || (host ? `${host}/` : `${String(origin || "").replace(/\/$/, "")}/v1/software`),
...
function llmsTxt(origin, env = {}) {
  ...
  const lines = [
    ...
    "## Version history",
    "",
    "Append 1.9 / AZRT-1.9-CLOSE-1.0 / 2.0.0-rc1 and later notes here. Do not replace ## What this is.",
    ...
    `Edge MCP gateway: POST /mcp is THE public MCP surface. A tool call enters FragGate first, then runs. Diagnostics: fraggate_list, fraggate_describe, fraggate_call. No backdoor exec. POST /p/{slug}/{op} stays proxy-not-exec.`,
...
${distributionDoorsHtml(origin)}
  ...
  <div class="honesty">
    ...
    <ul>
      ...
      <li><strong>1.7.7</strong> lands AZCoherence (AZC-0.1) as a true in-process FragGate Softwares engine with <code>engine_digest</code>. Public FragGate ops: health, skill, doctor, verify, review_triad, alternate_score, coherence_check, neutralize_hallucination. invent_evidence / truth_score / akm_calibrate stay stub. Scoring-review placement (AZ-CLCE adjacent). Not a 34th MASTER-33 isolation software. Not AKM-TRIAD fabric. Product cite <a href="https://github.com/AzielEliab/AZCoherence">github.com/AzielEliab/AZCoherence</a>.</li>
      <li><strong>1.7.6</strong> syncs 4DMap (4DM-WP-1.0) in-process LIVE_OPS with product 0.2.0: pin, span, stack, gap, fork, walk, lens, class, cohort, absence, cap, join, list, example, plus frame_status, axis_describe, walk_trace, card_export, card_import, verify_chain, neighbor_cite. Stubs stay. <code>layer=inspection_frame</code>. Not a sequential gate and not an extra door. Catalog version 0.2.0. <code>engine_digest</code> is the runtime isolate artifact — GitBaby deploys the product Worker.</li>
      <li><strong>1.7.5</strong> Softwares capability wave 1: DecisionGATE / ForgeReceipts / TemporalLock / StaticClock / ChronoLock / TrajectoryLock / SpectralLock get rich health/skill plus domain-true doctor/verify/import_export (or window/gates/schema/targets). Catalog <code>one_line</code> is capability-forward. LIVE_OPS matches engine ops. Checklist <code>docs/audit/SUITE-CAPABILITY-CHECKLIST.md</code>. AKM-TRIAD-1.0 stays LIVE fabric (not Softwares-tab). 4DMap 1.7.4 inspection-frame ops stay. EmbryoLock / AZChat stay stubs.</li>
      <li><strong>1.7.4</strong> enhances 4DMap (4DM-WP-1.0) in-process LIVE_OPS: frame_status, axis_describe, walk_trace, card_export, card_import, verify_chain, neighbor_cite. Stubs stay. <code>layer=inspection_frame</code>. Not a sequential gate and not an extra door. Product 4dmap is still 0.1.0 on main — sync <code>engine_digest</code> after product 0.2.0 deploys.</li>
      ...
      <li><strong>1.7.2</strong> adds <code>GET /v1/azpipe/arch</code> (POST allowed as the same cite). Returns the locked MASTER-33 AZPIPE arch/strip — same payload FragGate already exposes as <code>pipeline</code> / <code>pipeline_strip</code>. Cite/read only. Not a Softwares-tab door. Not a FragGate slug.</li>
...
function staticPaths(origin, env = {}) {
  const paths = {
    ...
    "/v1/about": {
      get: {
        operationId: "catalog_about_v1",
        summary: "Alias of /about.",
        tags: ["catalog"],
        responses: { "200": { description: "About HTML" } },
      },
    },
  ...
  for (const [path, item] of Object.entries(src)) {
    ...
    for (const method of Object.keys(copy)) {
      ...
      if (opObj && typeof opObj === "object") {
        ...
        const extra = workerUrl(product, op)
          ? `Proxies to ${workerUrl(product, op)}.`
          : "In-runtime engine. POST /p/{slug}/{op} is still proxy-not-exec. Use POST /v1/fraggate/call.";
...
async function combinedOpenApi(request, env) {
  ...
  return {
    ...
    tags: [
      ...
      { name: "mesh", description: "QNM-BUILD-1.0 suite rollup. Read-only suite-presence ON by default. live/locked/isolated counts. Public disable of suite-presence is refused. Not a login mesh. Not a Softwares-tab product." },
...
async function upstreamFetch(env, product, path, init) {
  if (!product.worker && product.slug !== "aziel-corpus") {
    return { res: null, target: null, in_runtime: true };
  }
...
function healthBody(origin, env = {}) {
  ...
  return {
    ...
    mcp: "/mcp",
    sigil: "/sigil.png",
    ...discoveryHostFields((origin || CATALOG_HOST).replace(/\/$/, "")),
    kv_increment: false,
  };
}
...
function markDoorEntered(out) {
  if (!out || typeof out !== "object") return out;
  out.fraggate_entered = true;
  if (out.envelope && typeof out.envelope === "object") out.envelope.fraggate_entered = true;
  return out;
}
