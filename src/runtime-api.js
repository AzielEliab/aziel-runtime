...
export const VERSION_HISTORY = [
  { version: "2.0.0-rc1", status: "current", note: "Certification-point freeze (not a feature dump). Additive SpectralLock vendor sync after spectrallock#16 (0.3.1, 92fcc3b031b3c6e1bf398d4263092e20d9bad3c5): vendored overlay.js byte-for-byte; wheel paint plane separate from the spectral triad; FragGate LIVE_OPS add pigment and restore-pigment (listPigment / pigmentFromB64); restore lost pigment LIVE under SpectralLock and refuses SL-PIGMENT-GONE when evidence is gone; overlay/inject/unredact keep pigment_recovery false; AMOE is not a live product; unredact/recover/handwriting stay off the FragGate door; catalog version 0.3.1; rehashed engine_digest 16281860ab0787f2485ea2530fbbcecf968009da3ae1c4f2b6e689a15a9ad586 (azbot 7e0843602e8bfed071ab1a20aad32d6f22f8330be3526e29c29ffcfb16892b89). Additive SpectralLock vendor sync after spectrallock#13: leftover-bytes unredact + deep PDF revision_graph + per-revision copies + universal recover (SL-RECOVER-*; 7z/HEIC/HEIF SLOT) + handwriting (not ESDA/court cert; hosted JPEG SLOT) vendored in overlay.js from 4af8fcb5f8af57a627db1b1dbfe1b631240a6067; leftover recover is honest (object id/offset/stream); opaque refuse SL-UNREDACT-OPAQUE; never OCR-from-black-box; eleven LIVE modes + inject unchanged; rehashed engine_digest f4aea0fb891fe107fb4699d7e3692d65023d303e2508421d98711db7aad64149; LIVE_OPS/stubs unchanged (no invented FragGate unredact/recover/handwriting door; product Worker /v1/unredact /v1/recover /v1/handwriting). Additive SpectralLock vendor sync after spectrallock#12: leftover-bytes unredact (locate/lift/recover/refuse) vendored in overlay.js from 319b2003221064a5f56e222869c2fb1437a4a1b6; leftover recover is honest (object id/offset/stream); opaque refuse SL-UNREDACT-OPAQUE; never OCR-from-black-box; eleven LIVE modes + inject unchanged; rehashed engine_digest 3427dbcf2932b6bf4c6cf80735efd171b75519066e013db6d0df275c65989fb4; LIVE_OPS/stubs unchanged (no invented FragGate unredact door). Additive SpectralLock vendor sync after spectrallock#11: inject true|false (ON = false-color membership paint, not recovered pigment; OFF = gray of same gate; zero ignores) plus tazel_inband_pct/vyrn_inband_pct; eleven LIVE modes unchanged; rehashed engine_digest c66231df1acd91306c0a7318950794cdc565af3752ef969fa41591191750062e; LIVE_OPS/stubs unchanged; inject-ON/UV/balance-lemon-indent honesty. Additive SpectralLock vendor sync after spectrallock#10: eleven LIVE modes (adds candle/indent/lemon) and rehashed engine_digest fd93f811c01ecff930d0edc8efac1baf96c96484c925b6aae62b1ed80c6119e7; LIVE_OPS/stubs unchanged; 256px preview honesty. Additive QNM-CHANNEL-PLANE-1.0 cite + AZNet functional pairing + AZVPN auto-bind: GET /v1/mesh channel_plane wifi/bluetooth/rf/photon ON (local qnm-node) and vpn/public_vpn/tunnel_concentrator true with default_vpn_backend azvpn auto_use true (GET cites only). AZNet pair_status reports order/token pair with AZBrowser (pairing ≠ tunnel; public VPN auto-binds AZVPN). No MCP tool add/remove. Additive AZCL-FOLD-TIP-1.0: FoldLock-packed library tip (index cite + sample MASTER + About Aziel); FragGate foldlock/pack-verify + aziel-corpus/tip-pack; full_library_in_process false. Additive AZRT-WORKER-LAUNCH-1.0: every Worker launch includes product-specific #hashtag parts plus the shared About Aziel + FoldLock corpus tip (docs/WORKER-LAUNCH.md). No MCP tool add/remove. Sentinel pass2 MCP honesty: preferred protocolVersion 2025-11-25 (still accepts 2025-06-18 and 2025-03-26); invalid protocol HTTP 400; Mcp-Session-Id echo; DELETE /mcp teardown (reuse 404); mutating MCP tools require confirm=true or dry_run=true (HTTP FragGate call unchanged). Public contract frozen under docs/2.0/: FragGate list→describe→call, MCP tool names, OpenAPI parity, health/version, engine_digest, live/stub/proxy-fallback. Compatibility policy, receipt schema, refusal contract, breaking-change policy. Clean-room reproducibility + external adversarial pack for independent reviewers (self-test ≠ third-party lab). Gate 4 includes Glama TDQS 5.0 tools/list metadata (descriptions/params/annotations; no rename; no behavior change) plus existing glama.json/GitHub topics. Read-only QNM suite-presence is ON by default; POST /v1/mesh/disable refuses MESH-DISABLE-REFUSED. Additive ACT-RECEIPT-1.0 fabric: after FragGate list/call, POST /mcp, and significant POST /v1/*, POST corpus /v1/receipts/append with x-aziel-receipt when RECEIPT_APPEND_TOKEN is set (fail-open; public chain on corpus /receipts; not a Softwares-tab product). Additive NO-LIE-NO-REWRITE-1.0 law (companion under CROSS-NETWORK-SURVIVAL-1.0; does not replace the machine tip): receipts that still hash; copies not all on one tunnel; no rewrite key; the network is never allowed to lie even to self-preserve (docs/designs/NO-LIE-NO-REWRITE-1.0.md; GET /v1/mesh cites no_lie / no_rewrite). Additive COLD-MULTI-SHELF-1.0 cite (GET /shelves) matches live corpus#96 /shelves honesty: Plane A = 5 published surfaces / 2 family radii / 1 independent live; Plane B Codeberg + archive.org PASS still SLOT (https://archive.org/details/aziel-lockset-tip + https://archive.org/details/aziel-lockset-tip_202609, same blast_radius); Framagit URL null Zenodo tip-pack SLOT (zenodo_live:false; CNS-ZENODO-NOT-LIVE); doi null; Plane C USB SLOT. Remain-Off-by-Design items stay off. FragGate remains THE single door. Additive REDLINE-2026-09-14: public-door map; refuse anonymous mutate; operator token header-only; Growth-ON Allow (no GPTBot Disallow); Cloudflare TLS cite (no client-side crypto claim); Cap-7 design_of hub_designs + resolves_to_hub false; attack sims refuse. Architecture-fit placements zkattest / mmconsensus / toolbench are in-runtime engines behind FragGate (isolation 33 unchanged; no new MCP tool; see docs/2.0/ARCHITECTURE-FIT-FIVE.md). OPERATOR LAW: public live_nodes is mesh size (active + inactive, not isolated); {slug}-worker roster is software_nodes and must not be used alone as that pill; downloads are not live. Crawler abstract stays lead copy. Identity Aziel Eliab only." },
  ...
  { version: "1.6.13", status: "superseded", note: "QNM-BUILD-1.0 suite rollup (companion to AIH-WP-1.1): GET /v1/mesh + /status expose live/locked/isolated counts only (no QNM-S, no leaderboard). GET never enables extra radios. Keep join/heartbeat/leave/nodes + optional hash-only broadcast (never a publish path). Not a login mesh; not Node Gate/IP panel; not login-recovery; not upload proxy. Full node process is local qnm-node/ (boot/chain/apg/bearers/outbox/phoenix/score/memorial/tethers); anon-broadcast is that process's sibling loopback module only. FragGate slug=mesh + MCP mesh_*. Current law (2.0.0-rc1): read-only suite-presence ON by default; POST /v1/mesh/disable refuses. AZMail mesh_* stays product-local. docs/NODE_MESH.md. 1.6.12 remains the live catalog + client update foundation." },
...
description: >-
  ...
  1.6.13 adds
  the QNM-BUILD-1.0 suite rollup (GET /v1/mesh live/locked/isolated; not a login mesh; full node is local qnm-node/).
...
Call Softwares (tools/list name Softwares). The door runs first. ChainLock, TemporalLock, and ForgeReceipts stamp when the call needs a ledger. \`runtime_software\` remains a tools/call alias. \`confirm=true\` when a call writes. \`dry_run=true\` previews and writes nothing. \`background=true\` returns Running until a receipt hash exists. Done only with that hash. \`tools/list\` stays 36. \`fraggate_list\`, \`fraggate_describe\`, \`fraggate_call\`, and \`chainlock_*\` stay for diagnostics. Ledger ops stamp ChainLock on the acts chain, then TemporalLock and ForgeReceipts (decisiongate_check, library_lookup, memory_observe, memory_resolve, memory_calibrate, mesh_join, mesh_enable, mesh_heartbeat, mesh_leave, mesh_broadcast, runtime_run, runtime_session_exec). \`fraggate_call\` reports those three from the pipe when a real hash exists. Lamb Lens, SweepGate, Sentinel, and RoseClock run inside that pipe. Reads do not stamp ChainLock, TemporalLock, or ForgeReceipts.
...
1. **Call Softwares (tools/list name Softwares).** The door runs first. ChainLock, TemporalLock, and ForgeReceipts stamp when the call needs a ledger. \`confirm=true\` writes. \`dry_run=true\` previews and writes nothing. \`background=true\` returns Running until a receipt hash exists.
...
HTTP \`POST /p/{slug}/{op}\` is still a **proxy**. Proxy without a session receipt is **not** exec. Public VPN auto-binds AZVPN (HTTPS/WS REAL; WireGuard/OpenVPN/L3 SLOT; origin-hiding false). \`GET /v1/mesh\` cites \`vpn=true\` and never opens a session. AZMail anonymous ring is FragGate LIVE_OPS only (default off; not SMTP). The suite QNM rollup (\`GET /v1/mesh\`, MCP \`mesh_*\`, FragGate \`slug=mesh\`) is live/locked/isolated counts plus read-only suite-presence ON by default. \`GET /v1/mesh\` never enables radios beyond that. Public \`POST /v1/mesh/disable\` refuses \`MESH-DISABLE-REFUSED\`. Cron or request-path fans out live Softwares product Workers (TTL 5 min). Not a login mesh. Views/MCP/downloads do not enter QNM-S. Full node process is local \`qnm-node/\`. Packet-transfer coding design is **QNS-CD-1.0** (photon QNS1 1.3 on local \`qnsd\`; \`GET /v1/qns\` cites only). Library host \`www.azielcorpuslibrary.net/runtime/v1/mesh/enable\` may return **409** \`library-default-off\` instead of Worker \`MESH-NEED-BEARER\` / \`MESH-BAD-BEARER\` — that overlay is host-side (aziel-corpus), not this Worker. GET never enables extra radios.
...
**1.7.10 = durable QNM Live Nodes.** Read-only suite-presence is ON by default. GET /v1/mesh never enables radios beyond that. Cron (\`*/2 * * * *\`) or request-path joins/heartbeats every live Softwares product Worker (\`node_id\` \`{slug}-worker\`, no \`|\`; TTL 5 min). Product Workers proxy \`/v1/mesh/*\` via \`AZIEL_RUNTIME\`. Not a login mesh. Not Node Gate. Not QNM-S.
**1.7.9 = AZCoherence cross-map.** Catalog + engine \`peers\` / \`cross_map\`: azclce (peer scorer), AKM-TRIAD (fabric neighbor — not merged), AZInterface (human UI), Softwares hubs (azieleliab.com, azielcorpuslibrary.net, godlock.uk), product Worker URL. azclce reciprocates. Domain stays null (scoring-review placement; same pattern as decisiongate/forgereceipts). LIVE_OPS health/skill/doctor/verify and azbot toolkit cite azcoherence.**1.7.8 = EmbryoLock true in-process engine.** Vault/Custody with ARK. LIVE_OPS health/skill/doctor/verify-hash/policy/limitation cite the AzielEliab/EmbryoLock Stealth+ v1.1 contract (\`Open Source Code\`). Wipe/scorch/unlock-after-fail stay FG-STUB on the public mesh — Never execute on the public mesh. Catalog surface live-with-local-destructive-boundary. Softwares worker_home https://embryolock-download-tracker.vibelock.workers.dev/. AZChat stays name-only stub.
**1.7.7 = AZCoherence (AZC-0.1).** True in-process FragGate Softwares engine. Second-pass triad coherence reviewer (primary vs alternate → PASS/FLAG/NEUTRALIZE/REFUSE). LIVE_OPS health, skill, doctor, verify, review_triad, alternate_score, coherence_check, neutralize_hallucination. Softwares-tab Plain A–Z. Placement scoring-review (AZ-CLCE adjacent). Not AKM-TRIAD fabric. Never invents evidence. Confidence ≠ truth. Product cite: https://github.com/AzielEliab/AZCoherence.**1.7.6 = 4DMap product 0.2.0 LIVE_OPS sync.** pin, span, stack, gap, fork, walk, lens, class, cohort, absence, cap, join, list, example plus frame_status, axis_describe, walk_trace, card_export, card_import, verify_chain, neighbor_cite. Stubs stay. Not a sequential gate and not an extra door. Catalog version 0.2.0. GitBaby merges and deploys.
**1.7.5 = Softwares capability wave 1.** Rich health/skill plus domain-true doctor/verify/import_export (or window/gates/schema/targets) on DecisionGATE, ForgeReceipts, TemporalLock, StaticClock, ChronoLock, TrajectoryLock, SpectralLock. Catalog \`one_line\` is capability-forward. LIVE_OPS matches engine ops. Checklist: \`docs/audit/SUITE-CAPABILITY-CHECKLIST.md\`. AKM-TRIAD-1.0 stays LIVE fabric (not Softwares-tab). 4DMap 1.7.4 inspection-frame ops stay.**1.7.4 = 4DMap inspection-frame capability bump.** LIVE_OPS add frame_status, axis_describe, walk_trace, card_export, card_import, verify_chain, neighbor_cite. Stubs stay. Not a sequential gate and not an extra door.
**1.7.3 = audit WARN copy/hint.** Refuse \`exist.mcp\` lists the live MCP tool set and points at \`POST /mcp tools/list\`. Public FragGate call stays open; session mutate requires token when configured. Softwares-tab \`count\` includes placements; isolation \`software_count\` is 33. 4DMap catalog copy is inspection frame / not an extra door. OpenAPI lists \`POST /mcp\`.**1.7.2 = GET /v1/azpipe/arch.** Cite/read surface for the locked MASTER-33 AZPIPE strip (same payload as \`GET /v1/fraggate\` \`pipeline\` / \`pipeline_strip\`). Not a Softwares-tab door. Not a FragGate slug.**1.7.1 = AKM-TRIAD-1.0.** Adaptive knowledge recollection over ChainLock learn: Bayesian calibration, deterministic 3-of-4 triad (E/C/P/B), derived rebuildable index. Memory APIs behind FragGate. Not a Softwares-tab product. Posterior ≠ truth. No history rewrite.
...
| GET | \`/workspace\` | Human workspace HTML (operator panel, dashboard, FragGate console, mesh). Same door as MCP. |
  ...
  return {
    ...
    "/v1/skill": {
      get: {
        ...
        summary: "Skill markdown: 1.7.9 cross-maps AZCoherence (peers azclce / AZInterface / AKM-TRIAD fabric neighbor; domain stays null). 1.7.8 lands EmbryoLock as a true in-process engine (live-with-local-destructive-boundary). 1.7.7 lands AZCoherence (AZC-0.1) as a true in-process FragGate Softwares engine. 1.7.6 syncs 4DMap LIVE_OPS with product 0.2.0. 1.7.5 Softwares capability wave 1 (decisiongate / forgereceipts / temporallock / staticclock / chronolock / trajectorylock / spectrallock). 1.7.4 enhances 4DMap LIVE_OPS (frame_status/axis_describe/walk_trace/card_export/card_import/verify_chain/neighbor_cite). 1.7.3 aligns audit WARN copy (exist.mcp → tools/list; public FragGate call; catalog count_note). 1.7.2 adds GET /v1/azpipe/arch (MASTER-33 cite). 1.7.1 adds AKM-TRIAD-1.0. 1.7.0 locks MASTER-33 (FragGate single door). 1.6.15 locked SUITE-PIPE. 1.6.14 adds 4DMap (4DM-WP-1.0). 1.6.13 aligns the QNM-BUILD-1.0 suite rollup. 1.6.12 adds GET /v1/software + client update check. Honest about 1.1.0 through 1.7.9.",
        tags: ["runtime"],
    ...
    "/mcp": {
      get: {
        ...
        summary: "Public MCP transport note. Edge MCP gateway: POST /mcp terminates into FragGate. auth: none (public).",
        tags: ["fraggate"],
      ...
      post: {
        ...
        summary:
          "JSON-RPC MCP-over-HTTP. This Worker POST /mcp is THE edge MCP gateway — it terminates into FragGate. tools/list is 36 live MCP tools. First call: Softwares (tools/list name Softwares). The door runs first. ChainLock, TemporalLock, and ForgeReceipts stamp when the call needs a ledger. Diagnostics: fraggate_list → fraggate_describe → fraggate_call. Install / Try on Glama is the discovery listing. Echoes Mcp-Session-Id and MCP-Protocol-Version. Preferred protocolVersion 2025-11-25 (still accepts 2025-06-18 and 2025-03-26). Invalid protocol is HTTP 400. Mutating tools require confirm=true or dry_run=true. Refuse exist.mcp points here. Proxy /p/{slug}/{op} is proxy. FragGate remains THE single door.",
        tags: ["fraggate"],
        requestBody: {
          required: true,
          content: {
      ...
      delete: {
        operationId: "mcp_session_teardown",
        summary:
          "Streamable HTTP session teardown. Send Mcp-Session-Id. 204 on close; 404 if missing/unknown/already dead. Reuse of that id on POST /mcp is 404. No fake SSE.",
        tags: ["fraggate"],
        responses: {
          "204": { description: "Session closed." },
          "400": { description: "Mcp-Session-Id required." },
          "404": { description: "Session not found or already torn down." },
        },
      },
    },
    "/.well-known/mcp/server-card.json": {
      get: {
        ...
        summary:
          "Honest MCP server card. Name aziel-runtime, version 2.0.0-rc1, streamable-http JSON-RPC POST /mcp, protocolVersion 2025-11-25 (still accepts 2025-06-18 and 2025-03-26), auth none (public). Homepage azieleliab.com/runtime. Support GitHub aziel-runtime. No invented Glama UUID or OAuth IdP.",
        tags: ["fraggate"],
    ...
    "/v1/mesh/enable": {
      post: {
        ...
        summary: "Declare an extra bearer. Empty {} refused. Rate-limited. Read-only suite-presence is already ON. Not a login mesh.",
        tags: ["mesh"],
        requestBody: {
          required: true,
          content: {
    ...
    "/v1/mesh/disable": {
      post: {
        ...
        summary: "Refused (MESH-DISABLE-REFUSED). Read-only suite-presence stays ON. Public disable of suite-presence is refused.",
        tags: ["mesh"],
        requestBody: { required: false, content: { "application/json": { schema: { type: "object" } } } },
        responses: { "400": { description: "MESH-DISABLE-REFUSED — suite-presence stays ON" } },
      },
    },
    "/v1/mesh/join": {
      post: {
    ...
    "/v1/mesh/site-presence": {
      get: {
        ...
        summary: "Cite the hub site-viewer contract and current site_live_viewers. Never writes. Never pulls hub /count. Paint live_nodes / rollup.mesh only. Never paint software_nodes or rollup.live. live_nodes_tip is the seal.",
        tags: ["mesh"],
    ...
    "/v1/azpipe/arch": {
      get: {
        ...
        summary:
          "MASTER-33 AZPIPE cite/read. Same arch() payload FragGate already exposes as pipeline / pipeline_strip (v, magic, locked, master, lambgate, fraggate_single_door, roseclock, hop list, 11 domains / 33 softwares). Not a Softwares-tab door. Not a FragGate slug.",
        tags: ["runtime"],
