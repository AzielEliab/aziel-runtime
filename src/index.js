/**
 * aziel-runtime 1.7.5 — Softwares capability wave 1 (decisiongate / forgereceipts / temporallock / staticclock / chronolock / trajectorylock / spectrallock) + 1.7.4 4DMap inspection-frame capability bump (frame_status/axis_describe/walk_trace/card_export/card_import/verify_chain/neighbor_cite; not an extra door) + 1.7.3 audit WARN copy/hint (exist.mcp → tools/list; public FragGate call; catalog count_note; 4DMap not an extra door) + 1.7.2 GET /v1/azpipe/arch MASTER-33 cite + AKM-TRIAD-1.0 + MASTER-33 (FragGate single door) + locked suite heritage (SUITE-PIPE-1.6.15) + 4DMap (4DM-WP-1.0) + QNM-BUILD-1.0 suite rollup (default OFF; not a login mesh) + live software catalog + client update check + GitHub auto-deploy + FragGate door + UI-op aliases + EmbryoLock name-only stub + AZHub + AZInterface + AZNet + AZBrowser + AZMail + PeaceLock + KV-backed API use trackers.
 *
 ...
 * GET  /v1/runtime.json       machine manifest: role=engine-runtime (1.7.5), door=fraggate
...
function llmsTxt(origin) {
  ...
  const lines = [
    ...
    `4DMap: FragGate only. POST /v1/fraggate/call { slug: "4dmap", op }. Four-axis inspection frame T/Δ/Γ/Π (4DM-WP-1.0). Research-domain inspection frame inside Internal Domain Layer after AZPIPE — not a sequential gate and not an extra door. LIVE_OPS include frame_status, axis_describe, walk_trace, card_export, card_import, verify_chain, neighbor_cite. Cited on the locked MASTER-33 pipeline. truth_score / lumen_panel / invent_mark / backdate_class stay stub. FragGate claims cite join types.`,
...
<body>
  ...
  <p>Feature-state audit (Remain-Off-by-Design; authoritative intentional-OFF vs gaps for 1.7.3+; not a Softwares-tab product, not a FragGate slug): <a href="${auditGithubUrl(FEATURE_STATE_AUDIT.file)}">${escapeHtml(FEATURE_STATE_AUDIT.id)}</a> · <a href="${auditGithubUrl(FEATURE_STATE_AUDIT.pdf)}">PDF</a> · <a href="${AUDIT_GITHUB_TREE}">docs/audit/</a>. Softwares waves must not enable mesh, ARK wipe, VeilLock remote inject, AZ-OS shell, 4DMap truth_score/invent_mark/backdate, or a public rollback API. Author: Aziel Eliab only.</p>
  ...
  return {
    ...
    info: {
      ...
      description:
        ...
        "1.6.14 adds 4DMap (4DM-WP-1.0) as a true in-process engine: four-axis inspection frame T/Δ/Γ/Π after AZPIPE — not a sequential gate and not an extra door. Call only via POST /v1/fraggate/call (or MCP fraggate_call) with { slug: \"4dmap\", op }. 1.7.4 LIVE_OPS add frame_status/axis_describe/walk_trace/card_export/card_import/verify_chain/neighbor_cite. truth_score/lumen_panel/invent_mark/backdate_class stay stub. FragGate claims cite join types. " +
