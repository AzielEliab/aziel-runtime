...
export const VERSION_HISTORY = [
  { version: "2.0.0-rc1", status: "current", note: "Certification-point freeze (not a feature dump). Public contract frozen under docs/2.0/: FragGate list→describe→call, MCP tool names, OpenAPI parity, health/version, engine_digest, live/stub/proxy-fallback. Compatibility policy, receipt schema, refusal contract, breaking-change policy. Clean-room reproducibility + external adversarial pack for independent reviewers (self-test ≠ third-party lab). No intentional behavioral breaks from 1.9.3. Remain-Off-by-Design items stay off. FragGate remains THE single door. New engines deferred to 2.1+. Crawler abstract stays lead copy. Identity Aziel Eliab only." },
...
**1.7.10 = durable QNM Live Nodes.** suite-presence is operator-enabled. GET /v1/mesh never enables. While enabled, cron (\`*/2 * * * *\`) or request-path joins/heartbeats every live Softwares product Worker (\`node_id\` \`{slug}-worker\`, no \`|\`; TTL 5 min). Product Workers proxy \`/v1/mesh/*\` via \`AZIEL_RUNTIME\`. Not a login mesh. Not Node Gate. Not QNM-S.
