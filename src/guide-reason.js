...
function flowAnswer(intent, cards) {
  ...
  if (intent === "fraggate") {
    return {
      topic: "fraggate",
      answer:
        "Click the domain tab for the Software, then Run on its card. That posts to POST /v1/fraggate/call with that card's slug and op. Agents use the same door: fraggate_list, then fraggate_describe, then fraggate_call. confirm=true writes. dry_run=true previews and writes nothing. background=true returns Running until a receipt hash exists. Seal on the interface desk and the AZBot Seal button still need the confirm box. This guide does not dispatch.",
      actions: [
