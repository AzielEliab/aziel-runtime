...
If the product homepage or hub shows **Live Nodes**, proxy the runtime kernel and display `live_nodes` / `rollup.live` (living downloaded Softwares instances). Tooltip: `live_nodes_note`. Do **not** show `software_nodes` (`{slug}-worker` fan-out) as Live Nodes. Downloads are not live.
...
While suite-presence is LIVE, aziel-runtime fans out join/heartbeat for live Softwares product Workers as **`software_nodes`** (TTL 5 min) on cron or request-path. GET still never enables. Public **Live Nodes** stay instance heartbeats only.
