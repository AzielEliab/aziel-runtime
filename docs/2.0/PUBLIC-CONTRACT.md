...
| GET | `/v1/mesh/az-generator` | Cap-7 semantic-bridge cite (MirageGrid; inherit designs only including azcorpus + azlibrary; `resolves_to_hub: false`; `name_may_change`; not ICANN) |
| GET | `/shelves`, `/v1/shelves` | COLD-MULTI-SHELF-1.0 cite matching corpus#96 `/shelves` (Plane A 5/2/1; Plane B SLOT; Plane C USB SLOT; `doi` null) |
