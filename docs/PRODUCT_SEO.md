...
COLD-MULTI-SHELF-1.0 (below the abstract): `GET /shelves` cites corpus#96 honesty. Plane A = 5 published surfaces / 2 family radii / 1 independent live. Plane B SLOT (Codeberg PASS still SLOT; archive.org + GitFlic URL null; Zenodo refused; `doi` null). Plane C USB SLOT. Runtime is not a sixth surface. Growth-ON crawlers Allow. No visible 15:20. Person `@id` https://www.azieleliab.com/#aziel.
