...
3. **MCP `initialize`** advertises preferred `protocolVersion: "2025-11-25"` (honest streamable HTTP: `Mcp-Session-Id`, `MCP-Protocol-Version`, `DELETE /mcp`). `"2025-06-18"` and `"2025-03-26"` remain accepted. `serverInfo.name` stays `"aziel-runtime"`. `capabilities.tools.listChanged` stays `false`. Invalid protocol is HTTP 400. Mutating MCP tools require `confirm=true` or `dry_run=true` (HTTP FragGate call is unchanged).
