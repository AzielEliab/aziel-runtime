...
import { resetAdaptiveForTests, sharedSnapshot } from "../src/jeeves-adapt.js";
...
assert.equal(guided.body.epistemology.layers.library.free_pass, false);
assert.equal(guided.body.epistemology.layers.other.free_pass, false);
assert.equal(guided.body.epistemology.layers.corpus.triad.schema, "aziel.triad.v0.3");
...
assert.deepEqual(guided.body.steps.map((step) => step.id), ["lamb_lens", "pull_corpus", "pull_library", "other_source", "triad"]);
...
const adaptA = await orchestrate({ call: "jeeves_help", q: adaptQ, confirm: true, helped: "#domain-tab-library" });
assert.equal(adaptA.status, 200);
assert.equal(adaptA.body.adaptive.stored, true);
assert.equal(adaptA.body.software_count, 42);
assert.equal(adaptA.body.software_tab, false);
assert.equal(adaptA.body.writes_public_chain, false);
assert.equal(adaptA.body.sealed, false);
assert.match(adaptA.body.receipt.hash, /^[a-f0-9]{64}$/i);
assert.equal(adaptA.body.adaptive.receipt_hash, adaptA.body.receipt.hash);
assert.equal(adaptA.body.receipt.request.includes(adaptQ), false);
const adaptB = await orchestrate({ call: "jeeves_help", q: adaptQ, confirm: true, helped: "#domain-tab-library" });
assert.equal(adaptB.body.adaptive.shown, true);
assert.equal(adaptB.body.adaptive.candidate.free_pass, false);
assert.equal(adaptB.body.adaptive.candidate.believed, false);
assert.equal(adaptB.body.adaptive.candidate.triad.schema, "aziel.triad.v0.3");
assert.equal(adaptB.body.adaptive.candidate.triad.final.score, null);
assert.match(adaptB.body.answer, /#domain-tab-library/);
assert.deepEqual(adaptB.body.steps.map((step) => step.id), ["lamb_lens", "pull_corpus", "pull_library", "other_source", "triad"]);
assert.equal(adaptB.body.epistemology.free_pass, false);
assert.equal(adaptB.body.believed, false);
assert.ok(["uncertain", "provisional", "conflict"].includes(adaptB.body.assertion));
const adaptSnap = JSON.stringify(await sharedSnapshot({}));
assert.equal(adaptSnap.includes(adaptQ), false);
assert.equal(adaptSnap.includes("alice@example.com"), false);
const adaptPii = await orchestrate({ call: "jeeves_help", q: "how do the domain tabs work alice@example.com", confirm: true });
assert.equal(adaptPii.status, 200);
assert.equal(adaptPii.body.adaptive.withheld, true);
assert.equal(adaptPii.body.adaptive.topic, null);
assert.equal(adaptPii.body.adaptive.pii_stored, false);
assert.equal(adaptPii.body.receipt.request.includes("alice@example.com"), false);
assert.equal(adaptPii.body.receipt.output.includes("alice@example.com"), false);
const adaptSnapPii = JSON.stringify(await sharedSnapshot({}));
assert.equal(adaptSnapPii.includes("alice@example.com"), false);
assert.equal((await sharedSnapshot({})).topics.domain_tabs.asks, 2);
assert.equal((await sharedSnapshot({})).withheld, 1);
assert.equal((await sharedSnapshot({})).last_receipt_hash, adaptPii.body.receipt.hash);
const adaptDry = await orchestrate({ call: "jeeves_help", q: adaptQ, dry_run: true, confirm: true, helped: "#domain-tab-library" });
assert.equal(adaptDry.body.stored, false);
assert.equal(adaptDry.body.receipt, null);
assert.equal(adaptDry.body.adaptive.stored, false);
assert.equal((await sharedSnapshot({})).topics.domain_tabs.asks, 2);
const guideAdapt = await orchestrate({ call: "learner_guide", q: adaptQ, confirm: true, helped: "#domain-tab-library" });
assert.equal(guideAdapt.body.memory.attempted, false);
assert.equal(guideAdapt.body.confirm_ignored, true);
assert.equal(guideAdapt.body.stored_notes, false);
assert.equal(guideAdapt.body.adaptive.stored, true);
assert.equal(guideAdapt.body.software_count, 42);
assert.equal(guideAdapt.body.software_tab, false);
assert.equal(guideAdapt.body.adaptive.receipt_hash, guideAdapt.body.receipt.hash);
assert.equal((await sharedSnapshot({})).desks.azai, 1);
