...
- **Download** — \`GET ${host}/v1/update/check?slug=&version=\` then fetch \`download_url\` (counted Worker \`/download\`), or \`GET ${host}/v1/pull/{slug}\`.
- **Upload / ingest / receipt** — \`fraggate_call\` (azbrowser \`airlock_ingest\`, peacelock \`upload_envelope\`, forgereceipts \`verify\`, miragegrid \`verify-receipt\` / \`bridge\`). Same ops on OpenAPI as proxy paths.
- **Cap-7 names** — \`GET ${host}/v1/mesh/az-generator\` or MirageGrid \`bridge\`. Inherit hub designs only. \`resolves_to_hub: false\`. Not ICANN aliases.
...
${compatibleClientsHtmlItems()}
  ...
  <ul>
    ...
    <li><strong>Download</strong> — <code>GET ${host}/v1/update/check?slug=&amp;version=</code> then fetch <code>download_url</code>, or <code>GET ${host}/v1/pull/{slug}</code></li>
    <li><strong>Upload / ingest / receipt</strong> — <code>fraggate_call</code> (azbrowser <code>airlock_ingest</code>, peacelock <code>upload_envelope</code>, forgereceipts <code>verify</code>, miragegrid <code>verify-receipt</code> / <code>bridge</code>)</li>
    <li><strong>Cap-7 names</strong> — <code>GET ${host}/v1/mesh/az-generator</code> or MirageGrid <code>bridge</code>. Inherit hub designs only. <code>resolves_to_hub: false</code>. Not ICANN aliases.</li>
  </ul>`;
}
