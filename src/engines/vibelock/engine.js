/**
 * VibeLock engine (port of workers/download-tracker/src/runtime.js scoring).
 * Risk assessment, not a liveness proof. No live mic. Author: Aziel Eliab.
 */
function runtimeJson(body, status = 200) {
  if (status && status !== 200 && body && typeof body === "object" && body.status == null) body.status = status;
  return body;
}
export const PRODUCT = "vibelock";
export const VERSION = "0.3.0";
export const MOTTO = "Sound can be forged. Pixels can be forged. Physics is harder to fake.";
export const LABEL = "Media authenticity advisory (audio, image, and video), not courtroom proof.";
export const LIMITATION = LABEL;
...
export function checksFromFeatures(f) {
