...
Call the Softwares tool. The door runs first. ChainLock stamps when the call needs a ledger. \`confirm=true\` when a call writes. \`dry_run=true\` previews and writes nothing. \`background=true\` returns Running until a receipt hash exists. Done only with that hash. \`tools/list\` stays 36. \`fraggate_list\`, \`fraggate_describe\`, \`fraggate_call\`, and \`chainlock_*\` stay for diagnostics. Ledger ops stamp the acts chain (decisiongate_check, library_lookup, memory_observe, memory_resolve, memory_calibrate, mesh_join, mesh_enable, mesh_heartbeat, mesh_leave, mesh_broadcast, runtime_run, runtime_session_exec). \`fraggate_call\` stamps the session chain inside the pipe when a real hash exists. Lamb Lens, SweepGate, Sentinel, TemporalLock, RoseClock, and ForgeReceipts run inside that pipe. Reads do not stamp ChainLock.
...
1. **Call the Softwares tool.** The door runs first. ChainLock stamps when the call needs a ledger. \`confirm=true\` writes. \`dry_run=true\` previews and writes nothing. \`background=true\` returns Running until a receipt hash exists.
