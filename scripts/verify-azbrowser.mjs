...
assert.match(product.oneLine, /Browse and search/i);
...
assert.match(product.description, /ethical research search/i);
...
for (const op of expectedLive) {
  ...
  const mcp = await post("/mcp", {
    ...
    id: 1,
    method: "tools/call",
    params: { name: "fraggate_call", arguments: { slug: "azbrowser", op, payload, confirm: true } },
  });
