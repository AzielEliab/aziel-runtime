...
If the product homepage or hub shows **Live Nodes**, proxy the runtime kernel and display `live_nodes` / `rollup.mesh` (human mesh users + cited `human_uses`). Tooltip: `live_nodes_note`. Do **not** show `software_nodes` (`{slug}-worker` roster) as Live Nodes. Downloads and Softwares instances are not live.
...
While suite-presence is LIVE, aziel-runtime fans out join/heartbeat for live Softwares product Workers as **`software_nodes`** (TTL 5 min) on cron or request-path. GET still never enables. Public **Live Nodes** counts human mesh users plus cited human uses (`USES`). `software_nodes` never feeds that pill.
