...
- Additive mesh channel-plane cite + AZNet functional pairing (not a new public MCP tool): `GET /v1/mesh` publishes `channel_plane` (wifi / bluetooth / rf / photon ON as cites; local qnm-node hardware; not Worker VPN). `vpn: false` / `origin_hiding: false` stay. AZNet `pair_status` reports order/token pair state with AZBrowser. Pairing ≠ tunnel. Human `#op-panel` / `#dashboard` / `#fg-console` stay. No MCP tool added or removed. See [`docs/NODE_MESH.md`](../NODE_MESH.md).
