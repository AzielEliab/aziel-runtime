/**
 * aziel-runtime 1.6.15 — locked suite pipeline (SUITE-PIPE-1.6.15) + 4DMap (4DM-WP-1.0) + QNM-BUILD-1.0 suite rollup (default OFF; not a login mesh) + live software catalog + client update check + GitHub auto-deploy + FragGate door + UI-op aliases + EmbryoLock name-only stub + AZHub + AZInterface + AZNet + AZBrowser + AZMail + PeaceLock + KV-backed API use trackers.
 *
 ...
 * GET  /v1/runtime.json       machine manifest: role=engine-runtime (1.6.15), door=fraggate
...
import { LOCKED_STRIP, arch as azpipeArch } from "./azpipe.js";
import {
  HOMEPAGE_KEYWORDS,
...
const PRODUCTS_RAW = [
  ...
  },
  {
    ...
    banner:
      "4DMap (4DM-WP-1.0): four-axis inspection frame T/Δ/Γ/Π. Domain Door / inspection layer after AZPIPE routes to isolated engines. Not a sequential gate. Not a truth score. Not a Lumen panel. Does not invent marks or backdate class. Neighbors TemporalLock / StaticClock / ChronoLock / TrajectoryLock / SpectralLock. FragGate claims cite join types. ChainLock may stamp walks. Author Aziel Eliab.",
  },
];
...
const ONE_LINE = {
  ...
  "4dmap": "4DMap (4DM-WP-1.0): four-axis inspection frame T/Δ/Γ/Π. Domain Door, not a sequential gate. FragGate only.",
};
...
function llmsTxt(origin) {
  ...
  const lines = [
    ...
    `4DMap: FragGate only. POST /v1/fraggate/call { slug: "4dmap", op }. Four-axis inspection frame T/Δ/Γ/Π (4DM-WP-1.0). Domain Door / inspection layer after AZPIPE — not a sequential gate. Cited at Domain Doors on the locked suite pipeline (SUITE-PIPE-1.6.15). truth_score / lumen_panel / invent_mark / backdate_class stay stub. FragGate claims cite join types.`,
    `Locked suite pipeline: ${LOCKED_STRIP}. Outbound reverses to Response/Receipt → ChainLock-OUT → StaticClock → TemporalLock → Domain Doors → AZPIPE → DecisionGATE → SweepGate → FragGate. LambGate is not a hop. FoldLock fld3-wire is internal to AZPIPE. Illegal reorder is refused.`,
...
<body>
  ...
  <div class="honesty">
    ...
    <ul>
      ...
      <li><strong>1.6.15</strong> locks the suite hop order (SUITE-PIPE-1.6.15): PUBLIC/UI/Agents → FragGate → SweepGate → ChainLock-IN → DecisionGATE → AZPIPE → Domain Doors (4DMap inspection) → TemporalLock → StaticClock → ChainLock-OUT → Response/Receipt. FragGate <code>fraggate_call</code> aligns. DecisionGATE after ChainLock-IN, before domain exec. ChainLock-IN stamps inbound; ChainLock-OUT stamps outbound receipts. TemporalLock + StaticClock are advisory envelope stamps. LambGate is not a hop. Illegal reorder is refused. Identity Aziel Eliab only.</li>
      <li><strong>1.6.14</strong> adds 4DMap (4DM-WP-1.0) as a true in-process engine with <code>engine_digest</code>. Public FragGate ops: health, skill, card_new, card_pin, card_span, card_join, card_walk, card_list, verify_hash. truth_score / lumen_panel / invent_mark / backdate_class stay stub. Four-axis inspection frame T/Δ/Γ/Π. Domain Door / inspection layer — not a sequential gate. Plain bucket. worker_home <code>https://4dmap-download-tracker.vibelock.workers.dev/</code>. mesh.enabled_default false. qns_cd pointer like peers. FragGate claims cite join types. 34 catalog slugs.</li>
    ...
    <ul>
      ...
      <li>4DMap is a four-axis inspection frame T/Δ/Γ/Π (4DM-WP-1.0). It is <em>not</em> a sequential gate, not a truth score, not a Lumen panel. It does not invent marks or backdate class. Domain Door / inspection layer after AZPIPE. Neighbors TemporalLock / StaticClock / ChronoLock / TrajectoryLock / SpectralLock. FragGate claims cite join types. ChainLock may stamp walks.</li>
    </ul>
  </div>
  ...
  <p id="pipeline"><strong>Locked suite pipeline</strong> (1.6.15 — not LambGate): <code>${LOCKED_STRIP}</code>. Outbound reverses: Response/Receipt → ChainLock-OUT → StaticClock → TemporalLock → Domain Doors → AZPIPE → DecisionGATE → SweepGate → FragGate. FoldLock fld3-wire stays internal to AZPIPE. SweepGate / ChainLock / AZPIPE are fabric, not Softwares-tab. 4DMap is cited at Domain Doors. Illegal reorder is refused.</p>
  <p>LIVE fabric (not Softwares-tab): AZPIPE, SweepGate, ChainLock, LOCKSET, packed catalog, <strong>QNS-CD-1.0</strong> (photon QNS1 1.3; local <code>qnsd</code>; Worker cites only), SUITE-PIPE-1.6.15. MCP <code>chainlock_*</code>. <code>GET /v1/mesh</code> never enables. <code>GET /v1/qns</code> cites the packet-transfer coding design — it does not proxy local via emit. UI=MCP. No Node Gate.</p>
...
async function combinedOpenApi(request, env) {
  ...
  return {
    ...
    info: {
      ...
      description:
        "1.6.15 locks the suite hop order (SUITE-PIPE-1.6.15): PUBLIC/UI/Agents → FragGate → SweepGate → ChainLock-IN → DecisionGATE → AZPIPE → Domain Doors (4DMap inspection) → TemporalLock → StaticClock → ChainLock-OUT → Response/Receipt. DecisionGATE after ChainLock-IN, before domain exec. LambGate is not a hop. " +
        "1.6.14 adds 4DMap (4DM-WP-1.0) as a true in-process engine: four-axis inspection frame T/Δ/Γ/Π. Domain Door / inspection layer after AZPIPE — not a sequential gate. Call only via POST /v1/fraggate/call (or MCP fraggate_call) with { slug: \"4dmap\", op }. LIVE_OPS card_new/card_pin/card_span/card_join/card_walk/card_list/verify_hash. truth_score/lumen_panel/invent_mark/backdate_class stay stub. FragGate claims cite join types. " +
        ...
        "Agent default exec is POST /v1/fraggate/call or MCP fraggate_call (CallEnvelope → FragGate → SweepGate → ChainLock-IN → DecisionGATE → AZPIPE → Domain Doors → TemporalLock → StaticClock → ChainLock-OUT → Response/Receipt). " +
