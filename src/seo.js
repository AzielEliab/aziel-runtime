...
export function runtimeAboutField(origin) {
  ...
  return {
    ...
    architecture: {
      ...
      nodemesh:
        "QNM read-only suite-presence is ON by default. GET /v1/mesh never enables radios beyond that. Public Live Nodes (live_nodes) count living downloaded Softwares instances (join/heartbeat; TTL 5 min) — not catalog count, not {slug}-worker software_nodes. Downloads are not live. Local qnm-node. Not a login mesh / VPN / Node Gate.",
