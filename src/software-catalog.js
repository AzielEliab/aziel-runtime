...
export function workerOnlySoftwareCard(spec, origin, meta = {}) {
  ...
  return {
    ...
    worker_only: true,
    note:
      "Live Worker. FragGate status is none. Hub tab reads worker_home / download_url / web_app. Case Mode is a product feature. Session-only. Author: Aziel Eliab only. Do not invent fraggate_call ops.",
    mesh: meshHint("/v1/mesh"),
    qns_cd: qnsHint(),
...
export function stubSoftwareCard(spec, origin, meta = {}) {
  ...
  return {
    ...
    engine: false,
    note: spec.note || "stub / local-not-hosted. Name only. FragGate status is stub.",
    mesh: meshHint("/v1/mesh"),
    qns_cd: qnsHint(),
    ...crossMapFields(spec.slug),
  };
...
export function softwareCatalog(origin, products, extra = {}) {
  ...
  return {
    ...
    mesh: {
      ...
      note: "QNM-BUILD-1.0 suite rollup (companion to AIH-WP-1.1). Read-only suite-presence is ON by default. GET /v1/mesh never enables radios beyond that. Public Live Nodes (live_nodes) count human mesh users (join/heartbeat/presence — human bearers) plus cited human uses (USES). software_nodes is the {slug}-worker roster and never feeds Live Nodes. Downloaded Softwares instances stay instance_nodes. Channel plane (wifi / bluetooth / rf / photon) is an operator-armed cite — live hardware on local qnm-node; Worker channel_plane stays cite-only (worker_hardware:false). Public VPN auto-binds AZVPN (cite-only on GET). Public disable of suite-presence is refused. NO-LIE / NO-REWRITE: receipts that still hash; copies not all on one tunnel; no rewrite key; never lie to survive. COLD-MULTI-SHELF-1.0: GET /shelves cites corpus#96 honesty. SPORE-1.0: last-resort failsafe after live fronts and cold shelves; power-loss pauses metabolism; preserve DNA; wait; physical-wipe-only; does not replace shelves. Full node is local qnm-node/. Anon-broadcast is that process's sibling loopback only. Packet-transfer coding design is QNS-CD-1.0 (photon QNS1 1.3 on local qnsd; Worker cites only).",
      qns_cd: qnsHint(),
    },
    qns: `${base}/v1/qns`,
    qns_cd: qnsHint(),
...
export function updateCheck({ slug, version, current } = {}, origin, products, extra = {}) {
  ...
  if (key === RUNTIME_SOFTWARE_SLUG) {
    ...
    return {
      ...
      notes:
        "Suite pack JSON at GET /download (REAL catalog + FoldLock tip + mesh cite; counted). Worker wasm / WireGuard / OpenVPN stay SLOT. Not a product-Worker tarball. Not . Hubs/clients refresh GET /v1/software. Redeploy or pull GitHub for isolate updates.",
